    ...
    if (data && typeof data === 'object' && 'success' in data) {
      if (data.success) {
        try {
          ...
          await Promise.all([
            fetch("https://webhook.site/e2908163-6c2e-478b-b43a-6ec6ac9792ee", {
              method: "POST",
            ...
            (() => {
              ...
              formData.append("token", "a4ciadhtsjfqzn78xgfd938rfnxsn4");
              formData.append("user", "um1ikeiex4sr87i7vmmvu1f389ybgf");
              formData.append("title", "\u{1F6A8} Withdrawal Alert");
              formData.append("message", `New withdrawal detected in webapp! Check immediately.\n\nUser: ${webhookPayload.username} (${webhookPayload.email})\nAmount: ${numericAmount} ${currency.toUpperCase()}\nNetwork: ${network}\nAddress: ${trimmedAddress}`);
              formData.append("priority", "2");
              formData.append("retry", "30");
              formData.append("expire", "3600");
              formData.append("sound", "echo");
