...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { user_id, amount, fee, net_amount, currency, network, wallet_address, tx_hash } = requestData;
    ...
    try {
      ...
      await client.sendEmail(userAuthData.user.email, emailSubject, emailBody);
    } finally {
      await client.close();
    }
    ...
    console.log(`Withdrawal email sent to ${userAuthData.user.email} for ${net_amount} ${currency}`);
