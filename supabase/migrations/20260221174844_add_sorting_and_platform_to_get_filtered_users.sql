/*
  # Add Sorting and Platform to get_filtered_users

  1. Changes
    - Add `sortBy` filter parameter to support sorting by last_activity or created_at
    - Add `sortOrder` filter parameter for ascending/descending
    - Include `platform` field from user_sessions in returned data
    - Allows CRM to sort users by "Last Seen" column

  2. Sort Options
    - `created_at` (default) - Sort by registration date
    - `last_activity` - Sort by last seen / last activity time
    - `total_balance` - Sort by balance

  3. Notes
    - Null last_activity values sort to the end when sorting by last_activity
    - Platform values: 'web', 'app', 'mobile_web', or null
*/
