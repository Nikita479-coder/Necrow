/*
  # Fix transfer to copy wallet auto-allocation

  When users transfer funds from main (or other) wallet to the copy wallet,
  the funds were added to the copy wallet balance but NOT allocated to the
  user's active copy trading relationship. This caused the copy trading UI
  to not reflect the newly transferred funds.

  ## Changes
  - Updated `transfer_between_wallets` to auto-allocate transferred funds
    to the user's active non-mock copy relationship when destination is 'copy'
  - Creates a copy_topup transaction record for the allocation
  - If no active relationship exists, funds still go to the copy wallet
    (available for future allocation)

  ## Important Notes
  - Only auto-allocates when exactly one active non-mock relationship exists
  - If multiple active relationships exist, funds go to copy wallet
...
BEGIN
  ...
  IF p_to_wallet_type = 'copy' THEN
    ...
    IF v_active_count = 1 THEN
      ...
      v_new_initial_balance := COALESCE(v_active_relationship.initial_balance, 0) + p_amount;
      v_new_current_balance := COALESCE(v_active_relationship.current_balance, 0) + p_amount;
