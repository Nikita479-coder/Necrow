/*
  # Send Deposit Confirmation Email Inside process_crypto_deposit_completion

  ## Problem
  The deposit confirmation email was only triggered from the nowpayments-ipn-callback
  Edge Function AFTER the database function completed. This meant:
  - The check-deposit-status polling path never sent the email
  - Email failures were silently swallowed
  - The email call was decoupled from the actual deposit logic

  ## Fix
  - Add a pg_net.http_post() call directly inside process_crypto_deposit_completion
  - The email is now sent as part of the deposit completion logic itself
  - Works regardless of which caller invokes the function (IPN callback or polling)
  - Only fires for newly completed deposits (not already-processed ones)

  ## Changes
  - Updated `process_crypto_deposit_completion` to call `send-deposit-confirmation-email`
    Edge Function via pg_net after crediting the wallet
  - Fetches the updated wallet balance to include in the email payload
  - Uses the project's Supabase URL and anon key for the HTTP call
*/

CREATE OR REPLACE FUNCTION public.process_crypto_deposit_completion(
  p_nowpayments_payment_id text,
  p_status text,
  p_actually_paid numeric,
  p_outcome_amount numeric
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  v_deposit crypto_deposits;
  v_wallet_updated boolean := false;
  v_credit_currency text;
  v_normalized_currency text;
  v_credit_amount numeric;
  v_usd_value numeric;
  v_market_price numeric;
  v_notification_id uuid;
  v_already_processed boolean := false;
  v_exclusive_result jsonb;
  v_multi_tier_result jsonb;
  v_deposit_bonus_result jsonb;
  v_new_balance numeric;
  v_email_request_id bigint;
BEGIN
  SELECT * INTO v_deposit
  FROM crypto_deposits
  WHERE nowpayments_payment_id = p_nowpayments_payment_id
  FOR UPDATE;

  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Deposit not found'
    );
  END IF;

  IF v_deposit.completed_at IS NOT NULL THEN
    v_already_processed := true;

    UPDATE crypto_deposits
    SET
      status = p_status,
      actually_paid = COALESCE(p_actually_paid, actually_paid),
      outcome_amount = COALESCE(p_outcome_amount, outcome_amount),
      updated_at = now()
    WHERE nowpayments_payment_id = p_nowpayments_payment_id;

    RETURN jsonb_build_object(
      'success', true,
      'already_processed', true,
      'deposit_id', v_deposit.payment_id,
      'status', p_status,
      'wallet_updated', false,
      'message', 'Deposit was already credited'
    );
  END IF;

  UPDATE crypto_deposits
  SET
    status = p_status,
    actually_paid = p_actually_paid,
    outcome_amount = p_outcome_amount,
    updated_at = now(),
    completed_at = CASE WHEN p_status IN ('finished', 'partially_paid', 'overpaid') THEN now() ELSE completed_at END
  WHERE nowpayments_payment_id = p_nowpayments_payment_id;

  IF p_status IN ('finished', 'partially_paid', 'overpaid') AND p_actually_paid > 0 THEN
    v_credit_currency := v_deposit.pay_currency;
    v_normalized_currency := normalize_crypto_currency(v_credit_currency);
    v_credit_amount := p_actually_paid;

    IF p_outcome_amount > 0 THEN
      v_usd_value := p_outcome_amount;
    ELSE
      SELECT last_price INTO v_market_price
      FROM market_prices
      WHERE pair = v_normalized_currency || 'USDT'
      LIMIT 1;

      IF v_market_price IS NOT NULL AND v_market_price > 0 THEN
        v_usd_value := v_credit_amount * v_market_price;
      ELSE
        v_usd_value := v_deposit.price_amount;
      END IF;
    END IF;

    INSERT INTO wallets (user_id, currency, balance, wallet_type)
    VALUES (v_deposit.user_id, v_normalized_currency, v_credit_amount, 'main')
    ON CONFLICT (user_id, currency, wallet_type)
    DO UPDATE SET
      balance = wallets.balance + v_credit_amount,
      updated_at = now();

    v_wallet_updated := true;

    SELECT COALESCE(w.balance, 0) INTO v_new_balance
    FROM wallets w
    WHERE w.user_id = v_deposit.user_id
      AND w.currency = v_normalized_currency
      AND w.wallet_type = 'main';

    INSERT INTO transactions (
      user_id,
      transaction_type,
      amount,
      currency,
      status,
      details
    ) VALUES (
      v_deposit.user_id,
      'deposit',
      v_credit_amount,
      v_normalized_currency,
      'completed',
      jsonb_build_object(
        'payment_id', v_deposit.payment_id,
        'nowpayments_payment_id', p_nowpayments_payment_id,
        'original_currency', v_deposit.pay_currency,
        'normalized_currency', v_normalized_currency,
        'pay_amount', p_actually_paid,
        'usd_value', v_usd_value,
        'wallet_type', 'main'
      )
    );

    INSERT INTO notifications (user_id, type, title, message, read)
    VALUES (
      v_deposit.user_id,
      'deposit_credited',
      'Deposit Credited',
      'Your deposit of ' || v_credit_amount || ' ' || UPPER(v_normalized_currency) || ' has been credited to your Spot wallet.',
      false
    )
    RETURNING id INTO v_notification_id;

    BEGIN
      UPDATE user_profiles
      SET
        ftd_at = now(),
        ftd_amount = v_usd_value,
        ftd_deposit_id = v_deposit.payment_id
      WHERE id = v_deposit.user_id
        AND ftd_at IS NULL;
    EXCEPTION WHEN OTHERS THEN
      NULL;
    END;

    BEGIN
      v_exclusive_result := distribute_exclusive_deposit_commission(
        v_deposit.user_id,
        v_usd_value,
        v_deposit.payment_id
      );
    EXCEPTION WHEN OTHERS THEN
      v_exclusive_result := jsonb_build_object('error', SQLERRM);
    END;

    BEGIN
      v_multi_tier_result := distribute_multi_tier_deposit_commissions(
        v_deposit.user_id,
        v_usd_value,
        v_deposit.payment_id
      );
    EXCEPTION WHEN OTHERS THEN
      v_multi_tier_result := jsonb_build_object('error', SQLERRM);
    END;

    BEGIN
      v_deposit_bonus_result := award_deposit_bonus(
        v_deposit.user_id,
        v_usd_value,
        v_deposit.payment_id
      );
    EXCEPTION WHEN OTHERS THEN
      v_deposit_bonus_result := jsonb_build_object('error', SQLERRM);
    END;

    BEGIN
      SELECT net.http_post(
        url := 'https://xcfyfzhcgphmiqvdfhrf.supabase.co/functions/v1/send-deposit-confirmation-email',
        headers := jsonb_build_object(
          'Content-Type', 'application/json',
          'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhjZnlmemhjZ3BobWlxdmRmaHJmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjIwODQ0NjUsImV4cCI6MjA3NzY2MDQ2NX0.74RgE-bEtfKxT8glAaYMG1Bm-doZ_oSf6wOwMoQPxqc'
        ),
        body := jsonb_build_object(
          'user_id', v_deposit.user_id::text,
          'deposit_amount', v_usd_value,
          'pay_currency', v_deposit.pay_currency,
          'new_balance', v_new_balance,
          'network', v_deposit.pay_currency,
          'wallet_type', 'Spot',
          'transaction_id', p_nowpayments_payment_id,
          'deposit_date', now()::text
        ),
        timeout_milliseconds := 10000
      ) INTO v_email_request_id;

      RAISE LOG 'Deposit confirmation email queued for user_id: %, request_id: %', v_deposit.user_id, v_email_request_id;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'Failed to queue deposit confirmation email for user_id: %, error: %', v_deposit.user_id, SQLERRM;
    END;
  END IF;

  RETURN jsonb_build_object(
    'success', true,
    'already_processed', false,
    'deposit_id', v_deposit.payment_id,
    'status', p_status,
    'wallet_updated', v_wallet_updated,
    'original_currency', v_credit_currency,
    'normalized_currency', v_normalized_currency,
    'amount_credited', v_credit_amount,
    'usd_value_calculated', v_usd_value,
    'notification_id', v_notification_id,
    'exclusive_affiliate', v_exclusive_result,
    'multi_tier_commissions', v_multi_tier_result,
    'deposit_bonus', v_deposit_bonus_result
  );
END;
$function$;
