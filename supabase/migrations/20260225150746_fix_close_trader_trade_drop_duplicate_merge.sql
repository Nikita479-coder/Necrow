/*
  # Fix close_trader_trade ambiguous function overloads

  ## Problem
  Two overloads of `close_trader_trade` exist with the same parameter types
  but in different order, causing PostgreSQL to fail with:
  "Could not choose the best candidate function"

  ## Fix
  - Drop both overloads
  - Recreate a single merged version that combines:
    - Mock/real balance distinction (from Feb 24 fix)
    - Futures position closing (from Feb 19 fix)
    - Cumulative PnL tracking
    - Wallet type based on mock vs real
*/

DROP FUNCTION IF EXISTS close_trader_trade(uuid, numeric, numeric, uuid);
DROP FUNCTION IF EXISTS close_trader_trade(uuid, uuid, numeric, numeric);
