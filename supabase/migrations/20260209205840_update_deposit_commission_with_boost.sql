/*
  # Update Deposit Commission Distribution with Recruitment Boost

  ## Summary
  Updates `distribute_exclusive_deposit_commission` to apply the rolling 30-day
  recruitment boost to deposit commissions. The boost multiplier is calculated
  live for each affiliate in the upline chain and recorded per commission entry.

  ## Changes
  - Calls `get_exclusive_affiliate_boost()` for each affiliate in the chain
  - Calculates base commission, then applies boost multiplier
  - Stores `base_commission_amount`, `boost_multiplier`, and `boost_tier` per record
  - `commission_amount` now holds the final boosted amount
  - Notification messages mention the boost when active

  ## Important
  - Fee revenue share (trading_fee commissions) is NOT affected by this boost
  - Only deposit and copy profit commissions receive the boost
*/
...
BEGIN
  ...
  LOOP
    IF v_upline.deposit_rate > 0 THEN
      v_base_commission := ROUND(p_deposit_amount * (v_upline.deposit_rate / 100), 2);

      IF v_base_commission > 0 THEN
        ...
        INSERT INTO exclusive_affiliate_commissions (
          affiliate_id, source_user_id, commission_type, tier_level,
        ...
        VALUES (
          v_upline.affiliate_id, p_depositor_id, 'deposit', v_upline.tier_level,
          p_deposit_amount, v_upline.deposit_rate, v_commission_amount,
          v_base_commission, v_boost_multiplier, v_boost_tier,
          p_reference_id, 'deposit', 'credited'
        );
        ...
        VALUES (
          ...
          'You earned $' || TRIM(TO_CHAR(v_commission_amount, 'FM999999999.00'))
            || ' (Level ' || v_upline.tier_level
            || ' - ' || TRIM(TO_CHAR(v_upline.deposit_rate, 'FM999999999.##')) || '%'
            || CASE WHEN v_boost_multiplier > 1.0
                 THEN ' + ' || TRIM(TO_CHAR((v_boost_multiplier - 1.0) * 100, 'FM999999999')) || '% boost'
                 ELSE ''
               END
            || ') from a deposit in your network.',
          false
        );
