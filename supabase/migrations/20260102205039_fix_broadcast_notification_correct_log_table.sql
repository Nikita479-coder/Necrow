/*
  # Fix Broadcast Notification Function

  1. Changes
    - Use correct table name: admin_activity_logs instead of admin_action_logs
    - Use correct column names: action_type, action_description, metadata
...
BEGIN
  ...
  SELECT (is_admin = true OR EXISTS (
    SELECT 1 FROM admin_staff WHERE admin_staff.user_id = p_admin_id AND is_super_admin = true
  ))
  INTO v_is_admin
  FROM user_profiles
  WHERE user_profiles.id = p_admin_id;
  ...
  -- Log the admin action using correct table and columns
