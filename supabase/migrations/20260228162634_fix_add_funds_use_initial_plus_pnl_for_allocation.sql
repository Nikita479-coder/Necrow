/*
  # Fix Add Funds - Prevent Re-Adding Copy Trading Profits

  ## Problem
  Users could re-add their copy trading profits via the Add Funds modal because
  the available balance calculation used `current_balance`, which could fall
  out of sync with the actual committed funds (initial deposit + accumulated profits).

  ## Fix
  Changed allocation calculation from `SUM(current_balance)` to
  `SUM(initial_balance + cumulative_pnl)`. The `cumulative_pnl` column is
  reliably updated by a database trigger whenever trade allocations close,
  making it a trustworthy source for total profit tracking.

  Available = copy_wallet_balance - SUM(initial_balance + cumulative_pnl)

  This ensures profits already earned through copy trading cannot be
  double-counted as available funds for new allocations.

  ## Changes
  - Updated `add_funds_to_copy_trading` function to use `initial_balance + cumulative_pnl`
    for calculating total allocated funds
*/
...
BEGIN
  ...
  SELECT COALESCE(SUM(
    GREATEST(0, COALESCE(initial_balance::numeric, 0) + COALESCE(cumulative_pnl::numeric, 0))
