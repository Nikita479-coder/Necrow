/*
  # Fix close_trader_trade double-counting cumulative_pnl

  1. Problem
    - The `close_trader_trade` function updates `cumulative_pnl` and `total_pnl` directly
    - The trigger `update_cumulative_pnl_on_allocation_close` on `copy_trade_allocations` ALSO updates the same fields
    - Both fire during the same trade close, causing PNL to be doubled

  2. Fix
    - Remove `cumulative_pnl` and `total_pnl` updates from `close_trader_trade` for BOTH mock and real trade paths
    - The trigger handles PNL tracking automatically when allocation status changes to 'closed'

  3. Impact
    - All future trade closes will correctly track PNL once (via the trigger only)
    - Existing inflated data will be corrected in a separate migration
*/
