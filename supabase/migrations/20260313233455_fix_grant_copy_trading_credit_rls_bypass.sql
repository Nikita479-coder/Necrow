/*
  # Fix Copy Trading Credit Grant Function RLS Issue

  1. Changes
    - Add admin check at the beginning of the function
    - Use admin privileges to bypass RLS when querying user_profiles
    - Only admins can grant credits, so this is safe
  
  2. Security
    - Function already has SECURITY DEFINER
    - Checks if caller is admin before proceeding
    - Safely bypasses RLS for user profile lookup
*/
...
BEGIN
  ...
  -- Fetch username - RLS bypass is safe since we've verified admin above
  ...
  IF v_awarded_by IS NOT NULL AND v_awarded_by != p_user_id THEN
    INSERT INTO admin_activity_logs (
      ...
      'Granted $' || ROUND(p_amount, 2)::text || ' copy trading credit to ' || COALESCE(v_username, 'User'),
      p_user_id,
      jsonb_build_object(
        'credit_id', v_credit_id,
        'amount', p_amount,
