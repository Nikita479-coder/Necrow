/*
  # Fix place_futures_order to Use Correct VIP Table

  ## Problem
  Function references non-existent `user_vip_tracking` table.
  The actual table is `user_vip_status`.

  ## Changes
  1. Replace `user_vip_tracking` with `user_vip_status`
  2. Replace `current_vip_level` with `current_level`
  3. Replace `period_volume` and `all_time_volume` with `volume_30d`
  4. Update volume tracking to use `volume_30d`

  ## Impact
  Fixes "relation user_vip_tracking does not exist" error
*/
...
BEGIN
  ...
  IF p_order_type = 'market' THEN
    ...
    BEGIN
      ...
      -- Update volume in user_vip_status
