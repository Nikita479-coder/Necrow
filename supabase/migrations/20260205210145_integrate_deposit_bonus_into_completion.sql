/*
  # Integrate Deposit Bonus Into process_crypto_deposit_completion

  1. Changes
    - Adds a call to `award_deposit_bonus()` inside `process_crypto_deposit_completion()`
      after the wallet is credited and commissions are distributed
    - The deposit bonus result is included in the return JSON
    - Wrapped in BEGIN/EXCEPTION to prevent deposit bonus errors from blocking the deposit
  ...
  3. Important Notes
    - This is the key change that makes deposit bonuses actually fire automatically
    - The award_deposit_bonus function handles tier tracking and duplicate prevention
    - If all 3 tiers are claimed, it simply returns success=false with no side effects
*/
