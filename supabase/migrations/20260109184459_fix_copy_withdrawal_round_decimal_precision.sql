/*
  # Fix Copy Trading Withdrawal - Round Decimal Precision in Transaction Details

  ## Problem
  When stopping copy trading, the transaction details JSON contains numeric values with 
  excessive decimal precision (e.g., 23.550403617931600000000000000000000000000000000000).
  
  ## Solution
  Round all numeric values to 8 decimal places before storing in transaction details.
  This provides sufficient precision while keeping the data clean and readable.
*/
...
BEGIN
  ...
  IF v_withdraw_amount > 0 THEN
    ...
    IF v_withdraw_amount > 0 THEN
      ...
      -- Record transaction with properly rounded values
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'initial_balance', round(v_initial_balance, 8),
          'final_balance', round(v_current_balance, 8),
          'profit', round(v_profit, 8),
          'platform_fee', round(v_platform_fee, 8)
        )::text,
        now()
      );
    END IF;
