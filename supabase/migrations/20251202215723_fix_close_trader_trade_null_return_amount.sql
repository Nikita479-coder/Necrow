/*
  # Fix close_trader_trade - NULL Return Amount
  
  ## Problem
  If v_allocation.allocated_amount or v_follower_pnl is NULL, then
  v_return_amount becomes NULL, which causes the wallet balance update
  to set balance to NULL.
  
  ## Solution
  Add COALESCE to all numeric calculations to ensure NULL values are treated as 0.
*/
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO transactions (
      ...
      'completed',
      'Copy trade closed: ' || COALESCE(v_trade.symbol, 'Unknown'),
      NOW()
    );
  END LOOP;
