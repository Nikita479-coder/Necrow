/*
  # Integrate Deposit Commissions into Completion Process

  ## Summary
  Updates the process_crypto_deposit_completion function to distribute
  both exclusive affiliate AND regular multi-tier deposit commissions.

  ## Changes
  - Calls distribute_multi_tier_deposit_commissions after exclusive affiliate distribution
  - Ensures all affiliates in the network receive their deposit commissions
  - Returns information about both types of commissions distributed
...
BEGIN
  ...
  IF p_status IN ('finished', 'partially_paid', 'overpaid') AND p_actually_paid > 0 THEN
    ...
    -- Distribute exclusive affiliate deposit commissions (if applicable)
    BEGIN
    ...
    -- Distribute regular multi-tier deposit commissions (if applicable)
    BEGIN
...
COMMENT ON FUNCTION process_crypto_deposit_completion IS 
  'Processes NOWPayments callback, credits user Main wallet, and distributes both exclusive and multi-tier affiliate deposit commissions';