/*
  # Apply Zero Fee Promotion in Place Futures Order

  1. Changes
    - Check if user has active zero fee promotion before calculating fee
    - If promotion is active, set fee to 0
    - Otherwise use normal VIP tier fee rates
*/
...
BEGIN
  ...
  -- CHECK FOR ZERO FEE PROMOTION (KYC verified within 7 days)
  v_has_zero_fee_promo := check_user_zero_fee_active(p_user_id);

  IF v_has_zero_fee_promo THEN
    -- Zero fee promotion is active
    v_fee_rate := 0;
    v_fee_amount := 0;
  ELSE
    -- Use normal VIP tier fee rates
  ...
  IF p_order_type = 'market' THEN
    ...
    BEGIN
      ...
      -- Only record fee collection if there's an actual fee
