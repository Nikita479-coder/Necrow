/*
  # Fix respond_to_copy_trade - Create Trader Trade on First Accept

  1. Changes
    - When first follower accepts, create the trader_trades record
    - Link pending trade to the newly created trader_trades
    - Subsequent accepts just create allocations

  2. Purpose
    - Trader position only exists when at least one follower accepts
    - No phantom positions in Live Positions
    - Clean pending trade flow
*/
...
BEGIN
  ...
  IF v_pending_trade.trader_trade_id IS NOT NULL AND EXISTS (
  ...
  -- ACCEPT flow - create the trader_trades record if it doesn't exist
  IF v_pending_trade.trader_trade_id IS NULL THEN
    ...
    -- Update pending trade with the trader_trade_id
