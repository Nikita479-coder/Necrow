/*
  # Integrate Auto-Accept into Pending Trade Creation

  ## Summary
  Updates the create_pending_copy_trade function to automatically accept trades
  for followers who have enabled the 24-hour auto-accept feature.

  ## Changes
  - After creating notifications, check each follower for auto-accept eligibility
  - For eligible followers, call auto_accept_pending_trade immediately
  - Track auto-accepted count separately from manual accepts
*/

-- Add auto_accepted_count column to pending_copy_trades
ALTER TABLE pending_copy_trades 
ADD COLUMN IF NOT EXISTS auto_accepted_count integer DEFAULT 0;

-- Update create_pending_copy_trade to handle auto-accept
