/*
  # Fix Admin Process Withdrawal to Preserve Address/Network

  1. Problem
    - When approving withdrawal, address and network are overwritten with NULL
    - This happens because p_wallet_address and p_network default to NULL
    - The original address/network from the withdrawal request is lost
    
  2. Solution
    - Use COALESCE to preserve existing address/network if new ones aren't provided
...
BEGIN
  ...
  IF p_action = 'approve' THEN
    -- Use COALESCE to preserve existing address/network if not explicitly provided
    UPDATE transactions
    SET 
      status = 'completed',
