/*
  # Fix Place Futures Order - Add Referral Commission Distribution

  1. Changes
    - Add distribute_trading_fees call after recording the opening fee
    - Referrer will now get notified when their referral opens a position
*/
...
BEGIN
  ...
  -- Record transaction first (needed for referral commission FK)
  ...
  -- Distribute referral commission (sends notification to referrer)
  IF v_trading_fee > 0.0001 THEN
    PERFORM distribute_trading_fees(p_user_id, v_transaction_id, v_notional_value, v_trading_fee, p_leverage);
  END IF;
