/*
  # Update Stop and Withdraw Function with Bonus Forfeiture

  ## Overview
  Updates the stop_and_withdraw_copy_trading function to handle bonus forfeiture
  when users withdraw before the 30-day lock period ends.

  ## Bonus Forfeiture Logic
  When a user withdraws early (bonus_locked_until > now()):
  - Calculate bonus proportion: bonus_amount / initial_balance (e.g., 100/600 = 16.67%)
  - Calculate forfeited amount: current_balance * bonus_proportion
  - User receives: current_balance - forfeited_amount - platform_fee (on remaining profit)
  
  After 30 days (bonus vested):
  - User keeps everything (bonus + proportional profits)
  - Normal platform fee applies to all profits

  ## Changes
  - Added bonus forfeiture calculation when withdrawing during lock period
  - Updates copy_trading_bonus_claims table on forfeiture
  - Includes bonus status in return payload
*/
