/*
  # Update close_position to Track Dual Volume System

  ## Summary
  Updates close_position function to track trading volume separately based on fund source:
  - Locked bonus trading volume (requires 60+ minute duration) - counts toward bonus unlock
  - Real wallet trading volume (no duration requirement) - counts toward VIP status

  ## Changes
  1. Calculate volume contribution using calculate_volume_contribution()
  2. Update bonus_trading_volume_completed for locked bonus positions (if duration met)
  3. Update referral_stats volumes for real wallet positions (always)
  4. Check for auto-unlock of bonuses when volume requirement is met
  5. Send notification if bonus volume doesn't count due to short duration

  ## Logic
  - If position uses locked bonus margin:
    - Check if duration >= 60 minutes
    - If yes: add to bonus_trading_volume_completed
    - If no: send warning notification
  - If position uses real wallet margin:
    - Always add to referral_stats volume (VIP tracking)
  - Mixed positions: split proportionally

  ## Security
  - Existing SECURITY DEFINER maintained
  - Auto-unlock only if requirements fully met
*/
...
BEGIN
  ...
  -- Update locked bonus volume if applicable
  IF v_margin_from_locked > 0 THEN
    ...
    IF v_oldest_bonus_id IS NOT NULL THEN
      ...
      IF v_duration_met AND v_bonus_volume > 0 THEN
        ...
        IF v_bonus_unlockable THEN
          ...
          VALUES (
            ...
            false,
            jsonb_build_object(
              'locked_bonus_id', v_oldest_bonus_id,
              'action', 'unlock_bonus'
            )
          );
        END IF;
      ELSIF v_margin_from_locked > 0 AND NOT v_duration_met THEN
        -- Position was closed too early - send warning notification
        INSERT INTO notifications (user_id, type, title, message, is_read, metadata)
        VALUES (
          v_position.user_id,
          'warning',
      ...
      END IF;
    ELSE
      -- No active bonus found, treat as regular margin
      v_margin_from_regular := v_margin_from_regular + v_margin_from_locked;
      v_margin_from_locked := 0;
    END IF;
  END IF;

  -- Update real trading volume for VIP tracking (always counts, no duration requirement)
  IF v_real_volume > 0 THEN
    -- Update referral_stats with real trading volume
  ...
  -- Return margin to futures wallet
