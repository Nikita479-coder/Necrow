/*
  # Fix Referral Function Type Casting

  ## Issue
  The function is returning varchar but expecting text, causing a type mismatch error.

  ## Solution
  Cast all returned values to the correct types.
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(rs.total_volume_all_time, 0)::numeric as trading_volume,
    (rt.created_at >= v_enrolled_at)::boolean as eligible
