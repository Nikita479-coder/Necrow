/*
  # Fix Promo Code Amount Formatting - Preserve Integer Values

  1. Problem
    - The previous formatting was trimming trailing zeros from ALL numbers
    - This caused $20 to display as $2 (the '0' was being trimmed)

  2. Solution
    - Use CASE statement to check if amount is a whole number
    - Only trim trailing zeros when there's actually a decimal portion
    - For whole numbers like 20, 100, 500 - display as-is without trimming

  3. Impact
    - $20.00 will now correctly display as $20 (not $2)
    - $5.50 will display as $5.5
    - $100.00 will display as $100
*/
...
BEGIN
  ...
  IF v_promo_record.bonus_amount = floor(v_promo_record.bonus_amount) THEN
    v_formatted_amount := floor(v_promo_record.bonus_amount)::text;
  ELSE
    v_formatted_amount := trim(trailing '0' from to_char(v_promo_record.bonus_amount, 'FM999999999.99'));
  END IF;
