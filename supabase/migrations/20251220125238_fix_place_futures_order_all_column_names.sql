/*
  # Fix place_futures_order function with correct column names
  
  Fixes all column name mismatches:
  - market_prices: uses 'pair' and 'last_price' (not 'symbol' and 'price')
  - futures_positions: uses 'position_id' and 'margin_allocated' (not 'id' and 'margin_used')
  - futures_orders: uses 'order_id', 'order_status', 'margin_amount' (not 'id', 'status', 'margin_reserved')
  - futures_margin_wallets: uses 'available_balance' (not 'balance')
  - fee_collections: uses 'currency' (not 'fee_currency')
*/
...
BEGIN
  ...
  -- Get market price (using correct column: last_price, and pair)
  ...
  -- Check available balance (using correct column: available_balance)
  ...
  IF p_order_type = 'market' THEN
    -- Deduct from wallet (using correct column: available_balance)
    UPDATE futures_margin_wallets
    SET available_balance = available_balance - v_total_cost,
        updated_at = now()
    WHERE user_id = p_user_id;

    -- Record fee (using correct column: currency)
    INSERT INTO fee_collections (
      user_id, fee_type, pair, fee_amount, currency
    ) VALUES (
      p_user_id, 'futures_open', p_pair, v_fee_amount, 'USDT'
    );

    -- Create position (using correct column: margin_allocated)
  ...
  ELSE
    -- Create limit order (using correct columns: order_status, margin_amount)
    ...
    -- Reserve margin (using correct column: available_balance)
    UPDATE futures_margin_wallets
