/*
  # Fix get_filtered_users auth schema access

  1. Changes
    - Recreate function with search_path including auth schema
    - Ensures last_sign_in_at and email are returned from auth.users
*/

DROP FUNCTION IF EXISTS get_filtered_users(json, int, int);
...
DECLARE
  ...
  v_sort_order text;
BEGIN
  IF NOT public.check_admin_permission('view_users') THEN
  ...
  LEFT JOIN public.vip_tier_tracking vt ON vt.user_id = up.id
  LEFT JOIN public.user_sessions us ON us.user_id = up.id
  WHERE (v_search IS NULL OR v_search = '' OR up.username ILIKE '%' || v_search || '%' OR up.id::text ILIKE '%' || v_search || '%')
    ...
    AND (
      ...
      (v_has_deposits = 'true' AND EXISTS (
        SELECT 1 FROM public.crypto_deposits cd 
  ...
  FROM (
    ...
    ORDER BY
      ...
      CASE WHEN v_sort_by = 'total_balance' AND v_sort_order = 'desc' THEN (
        SELECT COALESCE(SUM(w.balance::numeric), 0) FROM public.wallets w WHERE w.user_id = up.id AND w.currency = 'USDT'
      ) END DESC NULLS LAST,
