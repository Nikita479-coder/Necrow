/*
  # Fix place_futures_order to Use Correct Fee Tables

  ## Problem
  Function references non-existent `fee_configurations` table.
  Should use `trading_fee_tiers` table instead.

  ## Solution
  Update function to:
  1. Query `trading_fee_tiers` for user's VIP level
  2. Use taker_fee_rate from the tiers table
  3. Default to 0.0006 (0.06%) if not found

  ## Impact
  Fixes "relation fee_configurations does not exist" error
*/
...
BEGIN
  ...
  -- Get user's VIP level for fee calculation
  SELECT COALESCE(uvt.current_vip_level, 1) INTO v_user_vip_level
  FROM user_vip_tracking uvt
  WHERE uvt.user_id = p_user_id;

  IF v_user_vip_level IS NULL THEN
    v_user_vip_level := 1;
  END IF;

  -- Get taker fee rate from trading_fee_tiers
  SELECT tft.taker_fee_rate INTO v_fee_rate
  FROM trading_fee_tiers tft
  WHERE tft.vip_level = v_user_vip_level;

  -- Default to 0.06% if not found
    ...
    BEGIN
      ...
      IF v_fee_amount > 0 THEN
        INSERT INTO fee_collections (
          ...
          p_user_id, 'taker', v_fee_amount, v_position_size, p_pair, v_fee_rate, 'USDT'
        )
