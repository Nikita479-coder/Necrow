/*
  # Fix Close Trader Trade - Accurate PNL Calculation
  
  ## Changes
  This migration updates the close_trader_trade function to calculate follower PNL accurately:
  
  1. **Accurate PNL Calculation**:
     - Uses follower's allocated_amount and leverage
     - Calculates based on price difference (exit_price - entry_price)
     - Accounts for trade side (long/short)
     - Formula: position_size = (allocated_amount * leverage) / entry_price
     - PNL = position_size * (exit_price - entry_price) for long
     - PNL = position_size * (entry_price - exit_price) for short
  
  2. **Updated Balance Tracking**:
     - Returns allocated_amount + PNL to follower's wallet
     - Updates copy_relationship current_balance accurately
     - Tracks cumulative_pnl correctly
  
  3. **Proper Exit Price Recording**:
     - Records exit_price in allocations
     - Calculates PNL percentage based on actual price movement
*/
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      v_actual_pnl_percentage := (v_follower_pnl / NULLIF(v_allocation.allocated_amount, 0)) * 100;
