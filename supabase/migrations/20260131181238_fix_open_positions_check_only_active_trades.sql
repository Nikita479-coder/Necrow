/*
  # Fix Open Positions Check to Only Count Active Trader Trades

  ## Problem
  The withdrawal block was counting copy_trade_allocations where the user's allocation
  was 'open' but the trader's corresponding trade was already closed. This caused
  incorrect position counts.

  ## Solution
  Update the check to only count positions where BOTH:
  1. The user's copy allocation is 'open'
  2. The trader's corresponding trade is also 'open'

  This ensures only truly active positions block withdrawals.
*/
