/*
  # Fix Admin Support Tickets Function to Check User Profile

  1. Changes
    - Update `admin_get_support_tickets_with_users()` to check `user_profiles.is_admin` instead of JWT metadata
    - This ensures the function works immediately without requiring a session refresh
    
  2. Security
    - Still maintains admin check for security
    - Uses the user_profiles table which is the source of truth
*/
