/*
  # Update Pending Trade Functions for Percentage-Based Allocation

  ## Overview
  Updates database functions to support percentage-based position mirroring:
  - Store margin_percentage when creating trades
  - Calculate follower allocations based on their balance × percentage
  - Apply copy relationship multipliers if configured

  ## Changes
  1. `create_pending_copy_trade` - Accept and store margin_percentage parameter
  2. `respond_to_copy_trade` - Calculate allocation using follower's balance × percentage
  3. Both functions maintain backward compatibility

  ## Calculation Logic
  - Base allocation = follower_balance × margin_percentage / 100
  - If relationship has copy_amount setting, apply as multiplier
  - Example: 10% base × 50% relationship = 5% final allocation
*/

-- Drop and recreate create_pending_copy_trade with new parameter
DROP FUNCTION IF EXISTS create_pending_copy_trade(uuid, text, text, numeric, numeric, integer, numeric, text, numeric);
...
-- Drop and recreate respond_to_copy_trade with updated allocation logic
...
BEGIN
  ...
  IF v_relationship.copy_amount IS NOT NULL AND v_relationship.copy_amount > 0 THEN
    v_allocated_amount := v_allocated_amount * (v_relationship.copy_amount / 100.0);
  END IF;
