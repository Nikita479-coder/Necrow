/*
  # Fix Referral Tracking Functions
  
  1. Changes
    - Drop and recreate get_referrer_statistics with proper admin check
    - Drop and recreate get_referred_users_details with proper admin check
    - Fix email retrieval from auth.users
    - Ensure proper data aggregation from actual tables
  
  2. Security
    - Use direct profile check instead of overloaded is_user_admin()
...
-- Recreate get_referrer_statistics with fixed admin check
...
BEGIN
  -- Check if user is admin using direct profile check
  SELECT is_admin INTO current_user_profile
  FROM user_profiles
  WHERE id = auth.uid();
  
  IF current_user_profile.is_admin IS NOT TRUE THEN
  ...
  WITH referrer_users AS (
    SELECT 
      ...
      up.id as user_id,
      up.created_at,
      CASE WHEN COUNT(fp.id) > 0 THEN true ELSE false END as is_trader,
      ...
      COALESCE(rs.total_trading_volume_generated, 0) as trading_volume
    ...
    GROUP BY up.referred_by, up.id, up.created_at, rs.total_trading_volume_generated
  ),
  referrer_stats AS (
    SELECT
...
-- Recreate get_referred_users_details with fixed admin check
...
BEGIN
  ...
  SELECT
    ...
    up.country,
    up.created_at,
    CASE WHEN COUNT(DISTINCT fp.id) > 0 THEN true ELSE false END as is_trader,
    CASE WHEN SUM(CASE WHEN t.transaction_type = 'deposit' THEN t.amount ELSE 0 END) > 0 THEN true ELSE false END as has_deposited,
    COALESCE(SUM(CASE WHEN t.transaction_type = 'deposit' THEN t.amount ELSE 0 END), 0) as total_deposits,
    COALESCE(rs.total_trading_volume_generated, 0) as trading_volume,
  ...
  GROUP BY up.id, up.full_name, au.email, up.country, up.created_at, up.kyc_status, rs.total_trading_volume_generated
