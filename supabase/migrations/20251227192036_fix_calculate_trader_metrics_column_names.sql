/*
  # Fix calculate_trader_metrics Column Names
  
  1. Problem
    - Function references `realized_pnl` column from trader_trades table
    - Actual column name is `pnl` (not `realized_pnl`)
    - This causes "column 'realized_pnl' does not exist" error when closing trades
  
  2. Changes
    - Update all references from `realized_pnl` to `pnl` in trader_trades queries
    - Column mapping:
      - trader_trades: `pnl` and `pnl_percent`
      - copy_trade_allocations: `realized_pnl` and `pnl_percentage`
*/
