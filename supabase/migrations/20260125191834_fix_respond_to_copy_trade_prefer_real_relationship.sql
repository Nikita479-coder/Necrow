/*
  # Fix respond_to_copy_trade to prefer real relationships

  When a user has both a real and mock copy relationship to the same trader,
  the function should prefer the real (non-mock) relationship to ensure
  the trade is copied to the actual copy wallet.

  This fixes the issue where users with dual relationships weren't getting
  their trades copied because the function randomly picked the mock relationship
  which had no corresponding wallet.
*/
...
BEGIN
  ...
  -- Select the relationship, preferring non-mock (real) relationships
  -- This ensures users with both mock and real relationships get the real one
  SELECT cr.*, 
  ...
  IF v_wallet_balance IS NULL OR v_wallet_balance <= 0 THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Insufficient balance in your ' || v_wallet_type || ' wallet'
    );
  END IF;
