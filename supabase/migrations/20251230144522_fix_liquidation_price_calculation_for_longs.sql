/*
  # Fix Liquidation Price Calculation for LONG Positions

  ## Issue
  The liquidation price calculation was producing WRONG values for LONG positions:
  - Entry: 88359.30
  - Stored Liq: 91855.52 (ABOVE entry - impossible for LONG!)
  - Correct Liq: ~88094 (BELOW entry)

  ## Root Cause
  The formula was incorrectly calculating liquidation prices, potentially using
  cross-margin logic that produced impossible values (liq above entry for LONG).

  ## Fix
  For LONG positions: Liquidation price = Entry * (1 - 1/leverage + MMR)
  For SHORT positions: Liquidation price = Entry * (1 + 1/leverage - MMR)

  LONG positions MUST have liquidation prices BELOW entry price.
  SHORT positions MUST have liquidation prices ABOVE entry price.

  ## Changes
  1. Recreate place_futures_order with correct liquidation formula
  2. Add validation to ensure liquidation prices are logically correct
  3. Fix any existing positions with incorrect liquidation prices
*/

-- First, fix any existing open positions with incorrect liquidation prices
UPDATE futures_positions
SET liquidation_price = 
  CASE 
    WHEN side = 'long' THEN 
      entry_price * (1 - (1.0 / leverage) + 0.005)
...
-- Also fix closed positions for historical accuracy
UPDATE futures_positions
SET liquidation_price = 
  CASE 
    WHEN side = 'long' THEN 
...
-- Recreate place_futures_order with explicit validation
DROP FUNCTION IF EXISTS public.place_futures_order(uuid,text,text,text,numeric,integer,text,numeric,numeric,numeric,boolean,numeric);
...
BEGIN
  ...
  -- CRITICAL: Correct liquidation price calculation
  -- For LONG: liquidation is BELOW entry (price drop causes loss)
  -- For SHORT: liquidation is ABOVE entry (price rise causes loss)
  IF p_side = 'long' THEN
    v_liquidation_price := v_entry_price * (1.0 - (1.0 / p_leverage::numeric) + v_maintenance_margin_rate);
    -- Validate: LONG liquidation MUST be below entry
    IF v_liquidation_price >= v_entry_price THEN
      RAISE EXCEPTION 'Internal error: LONG liquidation price (%) must be below entry price (%)', v_liquidation_price, v_entry_price;
    END IF;
  ELSE
    v_liquidation_price := v_entry_price * (1.0 + (1.0 / p_leverage::numeric) - v_maintenance_margin_rate);
    -- Validate: SHORT liquidation MUST be above entry
    IF v_liquidation_price <= v_entry_price THEN
      RAISE EXCEPTION 'Internal error: SHORT liquidation price (%) must be above entry price (%)', v_liquidation_price, v_entry_price;
    END IF;
  END IF;
