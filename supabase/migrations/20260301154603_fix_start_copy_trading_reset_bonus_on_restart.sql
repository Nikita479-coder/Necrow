/*
  # Fix start_copy_trading restart to reset bonus_amount

  ## Problem
  When reactivating a stopped copy relationship, the UPDATE statement resets
  initial_balance, current_balance, cumulative_pnl, and total_pnl -- but does NOT
  reset bonus_amount or bonus_locked_until. The stale bonus_amount (e.g., 100 from a
  previously settled bonus) causes the profit-sharing fee to treat user capital as profit.

  ## Changes
  - In the restart section of start_copy_trading (overload with p_allocation_percentage),
    explicitly set bonus_amount = 0 and bonus_locked_until = NULL when reactivating
  - A new bonus is only applied if the user qualifies (v_bonus_eligible), which already
    handles this case correctly in the subsequent IF block

  ## Impact
  - Prevents stale bonus_amount from carrying over to restarted relationships
  - New bonuses are still awarded correctly when eligible
*/

CREATE OR REPLACE FUNCTION public.start_copy_trading(p_trader_id uuid, p_allocation_percentage integer, p_leverage integer DEFAULT 1, p_stop_loss_percent numeric DEFAULT NULL::numeric, p_take_profit_percent numeric DEFAULT NULL::numeric, p_is_mock boolean DEFAULT false, p_require_approval boolean DEFAULT false)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
