/*
  # Fix Fee Rebate Bug - Include Position ID in Fee Collection

  ## Problem
  The fee rebate trigger checks `position_id` to determine if the trade uses locked bonus.
  But `place_futures_order` wasn't including `position_id` in the fee_collections insert,
  so the trigger couldn't check and always applied the rebate.

  ## Solution
  Include `position_id` when inserting into fee_collections so the trigger can properly
  skip rebates for locked bonus trades.
*/
...
BEGIN
  ...
  IF p_order_type = 'market' THEN
    ...
    BEGIN
      ...
      -- IMPORTANT: Include position_id so fee rebate trigger can check locked bonus
