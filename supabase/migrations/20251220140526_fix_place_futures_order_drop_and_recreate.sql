/*
  # Fix Place Futures Order - Drop and Recreate with Correct Signature

  Drop existing function and recreate to match frontend parameters.
  Frontend passes quantity, not margin_usdt.
*/

DROP FUNCTION IF EXISTS public.place_futures_order(uuid,text,text,text,numeric,integer,numeric,numeric,numeric,numeric);
...
BEGIN
  ...
  -- Calculate trading fee BEFORE deducting margin
