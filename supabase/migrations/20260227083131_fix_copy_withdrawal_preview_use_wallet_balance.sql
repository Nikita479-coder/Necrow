/*
  # Fix copy trading withdrawal preview and execution to use actual wallet balance

  ## Problem
  The preview function (`calculate_copy_trading_early_withdrawal`) computed the
  withdrawal amount from the relationship's `initial_balance + cumulative_pnl`.
  But if the user transferred funds out of the copy wallet via `transfer_between_wallets`,
  the actual wallet balance would be lower. This caused the preview to show a higher
  "You will receive" amount than what the user actually got.

  ## Changes
  1. Updated `calculate_copy_trading_early_withdrawal` to cap its calculation at the
     actual copy wallet balance and include `previously_withdrawn` in the response.
  2. Updated `stop_and_withdraw_copy_trading` to compute fee proportionally when
     the wallet balance is lower than expected, ensuring consistent behavior.

  ## Notes
  - The user's total received (prior transfers + final withdrawal) is still correct
  - This fix ensures the preview matches what the user will actually receive
*/

-- Fix the preview function
CREATE OR REPLACE FUNCTION public.calculate_copy_trading_early_withdrawal(p_relationship_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Fix the actual withdrawal function
