/*
  # Fix Close Position Function - Use Correct Transaction Schema

  ## Description
  Updates close_position to use the correct transactions table schema.
  The table uses transaction_type (not type) and doesn't have a metadata column.

  ## Changes
  - Uses transaction_type instead of type
  - Removes metadata field (doesn't exist in schema)
  - Simplified transaction record for position close
*/
...
BEGIN
  ...
  INSERT INTO transactions (
    user_id, transaction_type, amount, currency, status, fee
  )
  VALUES (
    v_position.user_id,
    'futures_pnl',
