/*
  # Fix Get Affiliate Network By Level Function

  1. Changes
    - Fixed column name: user_profiles uses `id` not `user_id`
    - Fixed referred_by logic: it's a UUID referencing the referrer's user id, not a referral code
    
  2. Description
    - Correctly traverses the referral tree by following referred_by relationships
*/
...
BEGIN
  ...
  WITH RECURSIVE network AS (
    -- Level 1: Direct referrals (users who have referred_by = affiliate's user id)
    SELECT 
      up.id as user_id,
      1 as level,
