/*
  # Fix Admin Adjust Balance - Remove Admin Word from User Messages

  1. Changes
    - Update function to use generic terms without "admin"
    - Default messages: "Account credit" or "Balance adjustment"
    - Sanitize any input that contains "admin" word
*/
...
BEGIN
  ...
  -- Create generic user-facing message (remove any "admin" references)
  IF p_description IS NULL OR p_description = '' THEN
    v_user_facing_message := CASE 
      WHEN p_amount > 0 THEN 'Account credit'
      ELSE 'Balance adjustment'
    END;
  ELSE
    -- Sanitize the description to remove sensitive keywords AND "admin" word
    v_user_facing_message := p_description;
    
    -- Remove "admin" from user-facing message
    v_user_facing_message := REGEXP_REPLACE(v_user_facing_message, '\madmin\M', '', 'gi');
    v_user_facing_message := REGEXP_REPLACE(v_user_facing_message, 'by\s+', '', 'gi');
    v_user_facing_message := REGEXP_REPLACE(v_user_facing_message, 'from\s+', '', 'gi');
    v_user_facing_message := TRIM(v_user_facing_message);
    
    -- If message contains sensitive keywords, use generic message
    ...
    -- Fallback to generic if empty after sanitization
    IF v_user_facing_message = '' THEN
      v_user_facing_message := CASE 
        WHEN p_amount > 0 THEN 'Account credit'
        ELSE 'Balance adjustment'
      END;
  ...
  INSERT INTO transactions (
    ...
    abs(p_amount),
    'completed',
    v_user_facing_message,  -- Generic message visible to user (no "admin" word)
