/*
  # Integrate Consecutive Trade Tracking in close_position

  ## Summary
  Updates close_position function to call record_qualifying_trade when a position
  using locked bonus is closed. This enables tracking of consecutive trading days
  for the combined KYC + TrustPilot Review Bonus.

  ## Changes
  - After processing volume for locked bonus, call record_qualifying_trade
  - Pass position duration in minutes for the consecutive day tracking
  - The helper function handles the logic of:
    - Checking if position meets daily_trade_duration_minutes requirement
    - Updating daily_trade_count_today
    - Managing consecutive day streaks

  ## Note
  - Uses daily_trade_duration_minutes (15 min) for consecutive day tracking
  - This is separate from minimum_position_duration_minutes (10 min) for volume credit
*/
...
DECLARE
  ...
  v_consecutive_track_result JSONB;
BEGIN
  ...
  IF v_margin_from_locked > 0 THEN
    ...
    IF v_oldest_bonus_id IS NOT NULL THEN
      ...
      IF v_duration_met AND v_bonus_volume > 0 THEN
        ...
        IF v_bonus_unlockable THEN
          ...
          VALUES (v_position.user_id, 'bonus', 'Bonus Ready to Unlock!',
            'You have completed all requirements for your locked bonus! Visit your wallet to unlock it.',
      ...
      -- Track consecutive trading days for bonuses with that requirement
      -- Uses the daily_trade_duration_minutes from the bonus (e.g., 15 min for combined bonus)
      -- which is separate from the 10-minute volume requirement
      v_consecutive_track_result := record_qualifying_trade(
        v_position.user_id,
        v_oldest_bonus_id,
  ...
  RETURN jsonb_build_object(
    ...
    'consecutive_tracking', v_consecutive_track_result
  );
END;
$$;
