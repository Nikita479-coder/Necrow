/*
  # Update Close Position to Use Unified Commission Routing

  1. Changes
    - Updates close_position to use distribute_commissions_unified
    - This ensures closing fees respect the referrer's program choice (referral vs affiliate)
*/
...
BEGIN
  ...
  -- Record closing fee and distribute commissions
  IF v_closing_fee > 0.0001 THEN
    ...
    PERFORM distribute_commissions_unified(
      ...
      p_fee_amount := v_closing_fee,
      p_leverage := v_position.leverage
    );
  END IF;
