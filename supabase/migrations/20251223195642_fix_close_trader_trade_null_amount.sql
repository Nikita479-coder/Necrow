/*
  # Fix close_trader_trade - Handle NULL amounts

  1. Changes
    - Add COALESCE to all numeric calculations to prevent NULL values
    - Ensure transaction amount is never NULL
    - Handle edge cases where allocated_amount or pnl_percentage might be NULL

  2. Purpose
    - Fix error: null value in column "amount" of relation "transactions" violates not-null constraint
*/
...
BEGIN
  ...
  FOR v_allocation IN
    ...
    AND status = 'open'
  LOOP
    -- Calculate with COALESCE to prevent NULL
    ...
    -- Ensure we never insert NULL amounts
