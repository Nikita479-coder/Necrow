/*
  # Fix Admin Adjust Card Balance - Remove Description Field

  1. Changes
    - Remove description field from transaction insert
    - Transactions table only has: id, user_id, transaction_type, currency, amount, fee, status, tx_hash, address, network, confirmed_at, created_at, updated_at
    - Keep admin action logging for audit trail
...
-- Recreate without description field
...
BEGIN
  ...
  -- Create transaction record for audit trail (without description)
