/*
  # Fix Stop & Withdraw Copy Trading - Only Block on Active Trader Trades

  ## Problem
  The deployed function uses LEFT JOIN when checking for open positions,
  causing it to count copy_trade_allocations as "open" even when the
  corresponding trader_trade has already been closed. This blocks users
  from withdrawing.

  ## Solution
  1. Update `stop_and_withdraw_copy_trading` to use INNER JOIN with
     `tt.status = 'open'` so only truly active positions block withdrawal
  2. Update `calculate_copy_trading_early_withdrawal` with the same fix
  3. Clean up stale allocations where trader trade is closed but allocation
...
SET status = 'closed', updated_at = now()
WHERE cta.status = 'open'
  AND NOT EXISTS (
    SELECT 1 FROM trader_trades tt 
    WHERE tt.id = cta.trader_trade_id 
      AND tt.status = 'open'
  );
