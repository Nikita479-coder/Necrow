/*
  # Add last_sign_in_at to admin users list function

  1. Changes
    - Adds `last_sign_in_at` column to `get_admin_users_list` return type
    - Fetches the value from `auth.users` table (already joined)
    - Allows admin dashboard to display when users last signed in
*/
