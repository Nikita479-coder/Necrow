/*
  # Fix Deposit Commission to Use Actual USD Value

  1. Problem
    - When users deposit crypto, commissions were calculated on the crypto amount (e.g., 0.29 ETH)
      instead of the USD value ($584)
    - This resulted in tiny commissions ($0.04 instead of $70)

  2. Solution
    - Calculate the actual USD value of the crypto deposit using current market prices
    - Pass the USD value to commission functions instead of crypto amount
    - Use outcome_amount from NOWPayments if available, otherwise calculate from market price

  3. Changes
    - Add USD value calculation based on current market price
    - Pass v_usd_value to commission functions instead of v_credit_amount
...
BEGIN
  ...
  IF p_status IN ('finished', 'partially_paid', 'overpaid') AND p_actually_paid > 0 THEN
    ...
    -- First try to use outcome_amount from NOWPayments
    IF p_outcome_amount > 0 THEN
      v_usd_value := p_outcome_amount;
    ELSE
      -- Get current market price and calculate USD value
      ...
      ELSE
        -- Fallback to price_amount if no market price found
    ...
    -- Pass USD value to commission functions, not crypto amount
    BEGIN
