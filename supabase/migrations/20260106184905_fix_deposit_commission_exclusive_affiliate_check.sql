/*
  # Fix Deposit Commission Exclusive Affiliate Check

  ## Problem
  The function was checking for ea.status but the column is ea.is_active
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE up.id = p_depositor_id AND ea.is_active = true
  ) INTO v_skip_regular;
