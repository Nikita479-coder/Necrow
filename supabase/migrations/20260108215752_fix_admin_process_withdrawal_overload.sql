/*
  # Fix Admin Process Withdrawal Function Overloading

  1. Problem
    - Two versions of admin_process_withdrawal exist with different signatures
    - PostgREST cannot choose between them when called
...
DROP FUNCTION IF EXISTS admin_process_withdrawal(p_transaction_id uuid, p_action text, p_tx_hash text, p_admin_notes text);
DROP FUNCTION IF EXISTS admin_process_withdrawal(p_transaction_id uuid, p_action text, p_tx_hash text, p_admin_notes text, p_network text, p_wallet_address text);
