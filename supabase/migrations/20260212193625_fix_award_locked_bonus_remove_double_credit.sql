/*
  # Fix award_locked_bonus double credit bug

  1. Problem
    - The award_locked_bonus function was crediting the bonus amount BOTH to
      the locked_bonuses table AND to futures_margin_wallets.available_balance
    - This gave users double the intended funds: $X in locked bonus + $X as "real" wallet balance
    - The "real" wallet credit was treated as withdrawable funds by place_futures_order
      and close_position, allowing users to extract real money from bonus-only accounts

  2. Fix
    - Remove the INSERT/UPDATE to futures_margin_wallets from award_locked_bonus
    - The locked bonus should ONLY exist in the locked_bonuses table
    - place_futures_order already knows how to pull margin from locked_bonuses directly

  3. Impact
    - 939 locked bonuses were affected by the double credit
    - 245 users with zero deposits accumulated >$1 in extractable "real" balance
    - This migration only fixes the function going forward
    - A separate migration will correct existing balances
*/
...
CREATE OR REPLACE FUNCTION award_locked_bonus(
  ...
  p_expiry_days integer DEFAULT 0,
