/*
  ...
  ## Problem
  - Tier 2+ deposit commissions were going to main wallets
  - No records created in exclusive_affiliate_commissions table
  - No balance updates in exclusive_affiliate_balances table
  - Dashboard unable to show commission history

  ## Solution
  - Restore proper commission tracking for ALL tiers
  - Credit exclusive_affiliate_balances instead of main wallet
  - Create records in exclusive_affiliate_commissions for dashboard
  - Maintain network stats updates
  - Keep notifications working

  ## Changes
  1. Replace distribute_exclusive_deposit_commission function
  2. Ensure ALL tier levels create commission records
  3. Credit exclusive_affiliate_balances for withdrawable balance
  4. Update network stats for reporting
  5. Send notifications to affiliates

  ## Impact
  - Historical data remains unchanged
  - Future deposits will be properly tracked
  - Affiliates can see all commission tiers in dashboard
  - Balances show in exclusive affiliate wallet
*/
...
BEGIN
  -- Loop through all tiers in the upline chain
  FOR v_upline IN SELECT * FROM get_exclusive_upline_chain(p_depositor_id) LOOP
    IF v_upline.deposit_rate > 0 THEN
      ...
      IF v_commission_amount > 0 THEN
        -- 1. Create commission record in exclusive_affiliate_commissions table
        -- This enables dashboard tracking and reporting
        ...
        -- 4. Update tier-specific earnings counters
