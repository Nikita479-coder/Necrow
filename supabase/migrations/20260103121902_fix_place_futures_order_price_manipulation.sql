/*
  # CRITICAL SECURITY FIX: Remove Entry Price Manipulation Vulnerability

  ## Problem
  - place_futures_order accepts p_current_price from frontend
  - Users can send fake LOW entry prices to manipulate PnL
  - Example: User sent entry $2,992 when real price was $3,100, instant "profit"

  ## Solution
  - ALWAYS fetch entry price from market_prices table
  - NEVER trust frontend-provided prices for position entry
  - p_current_price parameter is now IGNORED for security

  ## Impact
  - Closes the remaining price manipulation vulnerability
  - Combined with close_position fix, all price vectors are now secure
*/

-- Drop all versions and recreate with security fix
...
CREATE OR REPLACE FUNCTION place_futures_order(
  ...
  p_current_price numeric DEFAULT NULL  -- KEPT FOR BACKWARDS COMPAT BUT IGNORED
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- CRITICAL SECURITY FIX: ALWAYS fetch price from database
  -- p_current_price parameter is IGNORED to prevent price manipulation
  SELECT mp.price INTO v_current_price
  FROM market_prices mp
  WHERE mp.symbol = p_pair;

  IF v_current_price IS NULL OR v_current_price <= 0 THEN
    -- Fallback: try with 'pair' column name
    ...
    BEGIN
      ...
      IF v_fee_amount > 0 THEN
        INSERT INTO fee_collections (
          user_id, fee_type, fee_amount, notional_size, pair, fee_rate, currency
        ) VALUES (
          p_user_id, 'futures_open', v_fee_amount, v_position_size, p_pair, v_fee_rate, 'USDT'
        )
