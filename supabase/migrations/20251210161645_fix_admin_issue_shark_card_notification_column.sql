/*
  ...
  ## Summary
  Fixes the admin_issue_shark_card function to use the correct column name 'read' 
  instead of 'is_read' when inserting notifications.

  ## Changes
  - Update admin_issue_shark_card function to use 'read' column instead of 'is_read'
  - Also adds 'shark_card_issued' to the notification type constraint if not present
*/

-- First, ensure the notification type exists in the constraint
DO $$
BEGIN
  -- Drop and recreate the constraint to include shark_card_issued
  ALTER TABLE notifications DROP CONSTRAINT IF EXISTS notifications_type_check;
  
  ALTER TABLE notifications ADD CONSTRAINT notifications_type_check 
    CHECK (type IN ('referral_payout', 'trade_executed', 'kyc_update', 'account_update', 'system', 'shark_card_issued', 'tpsl_triggered', 'tpsl_cancelled', 'withdrawal_blocked', 'withdrawal_unblocked', 'vip_level_upgraded'));
END $$;

-- Fix the admin_issue_shark_card function
...
BEGIN
  ...
  -- Create notification for user (using 'read' instead of 'is_read')
