/*
  # Fix Copy Wallet Withdrawal - Exclude Stopping Relationship

  ## Problem
  When withdrawing from a copy trading relationship, the frontend:
  1. Sets is_active = false on the relationship
  2. Calls transfer_between_wallets to move the funds

  But transfer_between_wallets still counts that relationship's funds as "allocated"
  even though is_active is false, because it may not have been committed yet or
  the query runs before the update is visible.

  ## Solution
  Update the ActiveCopyTrading frontend to properly handle the withdrawal by:
  - Using the stop_copy_trading function instead of manual updates
  - OR ensuring the relationship is properly excluded from allocation calculations

  For now, we'll update the check to ensure stopped relationships (status='stopped')
  are excluded even if is_active hasn't been updated yet.
*/

-- The issue is actually in the frontend, not the database function.
-- But let's add a safeguard: ensure that relationships with status='stopped' 
-- are treated as inactive even if is_active column hasn't been updated
  ...
  IF to_wallet_type_param = 'futures' THEN
    ...
    IF from_wallet_type_param = 'copy' THEN
      ...
      WHERE follower_id = user_id_param
        AND is_active = true
        AND status = 'active'  -- FIXED: Also check status
        AND is_mock = false;
