/*
  # Fix Deposit Completion - Add Exclusive Affiliate Commission

  ## Problem
  The deposit completion function was overwritten in later migrations and
  the call to distribute_exclusive_deposit_commission was removed.
  This caused exclusive affiliates to not receive their deposit commissions.

  ## Solution
  Recreate the process_crypto_deposit_completion function with:
  1. Correct wallet_type = 'main' (previously fixed)
  2. Call to distribute_exclusive_deposit_commission (re-added)
  3. All currency normalization features preserved
*/
...
BEGIN
  ...
  IF p_status IN ('finished', 'partially_paid', 'overpaid') AND p_actually_paid > 0 THEN
    ...
    -- Normalize currency (remove network suffixes like BSC, TRC20, etc.)
    v_normalized_currency := normalize_crypto_currency(v_credit_currency);
    
    v_credit_amount := p_actually_paid;

    -- Credit user's main wallet with normalized currency
    ...
    -- IMPORTANT: Distribute exclusive affiliate deposit commissions
    -- This was missing in previous versions causing affiliates to not receive commissions
    BEGIN
...
COMMENT ON FUNCTION process_crypto_deposit_completion IS 
  'Processes NOWPayments callback, credits user Main wallet, and distributes exclusive affiliate commissions';
