/*
  # Fix Withdrawal Function - Use Helper for Locked Bonus

  Fixes the `create_withdrawal_request` function to use the helper function
  `get_user_locked_bonus_balance()` instead of querying a non-existent column.

  ## Changes
  - Use get_user_locked_bonus_balance() to get locked bonus amount
  - Remove locked_bonus from wallet query
*/
...
BEGIN
  ...
  -- Get wallet balance and locked_balance for the currency
  SELECT 
    COALESCE(balance, 0),
  ...
  -- Get locked bonus balance using helper function
  v_locked_bonus := COALESCE(get_user_locked_bonus_balance(p_user_id), 0);
