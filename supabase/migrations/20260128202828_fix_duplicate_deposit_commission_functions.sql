/*
  # Fix duplicate deposit commission notifications
  
  1. Problem
    - Two versions of distribute_multi_tier_deposit_commissions exist
    - Text signature version only checks direct referrer for exclusive affiliate
    - UUID signature version checks entire upline chain
    - Both get called causing duplicate notifications for exclusive affiliates
  
  2. Solution
    - Drop both versions
    - Create single unified version that properly checks entire upline chain
    - Ensure function is called with correct parameter types
*/

-- Drop all versions of the function
DROP FUNCTION IF EXISTS distribute_multi_tier_deposit_commissions(uuid, numeric, text);
DROP FUNCTION IF EXISTS distribute_multi_tier_deposit_commissions(uuid, numeric, uuid);
...
BEGIN
  -- Check if depositor has ANY exclusive affiliate in their ENTIRE upline chain
  -- Skip regular commissions if true (regardless of is_active status)
  SELECT EXISTS (
    SELECT 1 
    FROM affiliate_tiers at
    JOIN exclusive_affiliates ea ON ea.user_id = at.affiliate_id
  ...
  IF v_skip_regular THEN
    RETURN jsonb_build_object(
      'success', true,
      'message', 'Skipped - User has exclusive affiliate in upline',
      'commissions_paid', 0,
      'total_distributed', 0
    );
  END IF;
