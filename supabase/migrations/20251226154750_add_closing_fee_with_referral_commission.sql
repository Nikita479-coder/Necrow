/*
  # Add Closing Fee with Referral Commission

  1. Changes
    - Update close_position function to charge a closing fee
    - Distribute referral commission when positions are closed
    - Send notification to referrer on close fee commission
  
  2. Fee Structure
    - Uses same fee rate as opening (from trading_pairs_config)
    - Referrer gets commission based on their VIP level
*/
...
BEGIN
  ...
  IF v_fee_rate IS NULL THEN
    v_fee_rate := 0.0004; -- Default 0.04%
  END IF;
  ...
  -- STEP 1: Return locked bonus margin to locked_bonuses table
  ...
  -- STEP 2: Return regular margin to futures wallet AND decrement locked_balance
  ...
  -- STEP 3: Handle net PnL (after closing fee)
  IF v_net_pnl >= 0 THEN
    UPDATE futures_margin_wallets
  ...
  -- STEP 4: Record closing fee and distribute referral commission
  IF v_closing_fee > 0.0001 THEN
    INSERT INTO fee_collections (
      user_id, fee_type, fee_amount, trade_amount, 
      pair, leverage, currency
    ) VALUES (
      v_position.user_id, 'futures_close', v_closing_fee, v_notional_value,
      v_position.pair, v_position.leverage, 'USDT'
    );
    
    -- Distribute referral commission (this sends notification to referrer)
    PERFORM distribute_trading_fees(v_position.user_id, v_closing_fee, v_notional_value);
  END IF;
