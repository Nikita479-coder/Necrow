/*
  # Add total_deposits to admin_users_list function

  1. Changes
    - Add total_deposits column to the returned data
    - Sums completed crypto deposits for each user
    
  2. Usage
    - Used by admin dashboard to filter users by deposit status
*/
...
BEGIN
  ...
  deposit_totals AS (
    SELECT 
      cd.user_id,
      SUM(COALESCE(cd.actual_amount, 0)) as total
