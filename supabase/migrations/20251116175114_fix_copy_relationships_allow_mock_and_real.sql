/*
  # Allow Separate Mock and Real Copy Trading

  1. Changes
    - Update unique constraint to include is_mock flag
    - Allows same trader to be followed in both mock and real mode separately
    - Update start_copy_trading to check for matching mode
    
  2. Notes
    - User can have one mock AND one real relationship with same trader
    - Cannot have duplicate relationships in same mode
*/

-- Drop old unique constraint
ALTER TABLE copy_relationships 
DROP CONSTRAINT IF EXISTS copy_relationships_follower_id_trader_id_key;

-- Add new unique constraint that includes is_mock
ALTER TABLE copy_relationships
ADD CONSTRAINT copy_relationships_follower_trader_mode_key 
UNIQUE (follower_id, trader_id, is_mock);

-- Update start_copy_trading function to check for same mode
