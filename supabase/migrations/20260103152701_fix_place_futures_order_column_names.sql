/*
  # Fix place_futures_order Column Names

  ## Problem
  The place_futures_order function has two critical errors:
  1. References non-existent column `tick_size` in trading_pairs_config
  2. Uses `tpc.symbol` instead of correct column name `tpc.pair`

  ## Solution
  - Drop existing function and recreate
  - Remove tick_size reference (column doesn't exist)
  - Change tpc.symbol to tpc.pair to match actual table schema

  ## Impact
  Fixes "column tpc.tick_size does not exist" error preventing order placement
*/
...
BEGIN
  -- FIXED: Use tpc.pair instead of tpc.symbol, removed tick_size
