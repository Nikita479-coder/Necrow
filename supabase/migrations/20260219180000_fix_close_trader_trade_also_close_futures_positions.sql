/*
  # Fix close_trader_trade to Also Close Follower Futures Positions

  ## Issue
  When an admin closes a trader's position, the function updates copy_trade_allocations
  and returns funds to wallets, but it doesn't close the follower's actual futures_positions.
  This causes positions to remain "open" in the UI even though the trade was closed.

  ## Changes
  - After processing each allocation, find and close any related futures_positions
  - This ensures followers don't see "ghost" open positions after trades close

  ## Impact
  - Follower positions will now properly sync with trader position closures
  - No more orphaned open positions after copy trades close
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Close any open futures positions for this follower related to this trade
    -- Match on user_id, pair, side, and similar entry price (within 1%)
    ...
    LOOP
      ...
      SET
        ...
        closed_at = NOW(),
        updated_at = NOW()
      WHERE id = v_futures_position.id;
    END LOOP;
