/*
  # Fix Stop and Withdraw Notification Column Name

  ## Issue
  The `stop_and_withdraw_copy_trading` function uses `notification_type` instead of `type`
  when inserting notifications, causing errors when users try to stop copy trading and withdraw.

  ## Fix
  Update the function to use the correct column name `type` instead of `notification_type`.
*/
...
BEGIN
  ...
  -- FIXED: Changed 'notification_type' to 'type'
  IF v_is_bonus_locked AND v_forfeited_amount > 0 THEN
