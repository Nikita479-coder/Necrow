/*
  # Update stop_and_withdraw to Track Credit Status

  1. Changes
    - When stopping a copy relationship that has linked copy_trading_credits:
      - If bonus is forfeited (before lock period): set credits to 'forfeited'
      - If bonus is kept (after lock period): set credits to 'used'
    - Also updates calculate_copy_trading_early_withdrawal to show credit info

  2. Why
    - The copy_trading_credits records need their status updated when a relationship ends
    - This allows proper tracking and prevents re-use of consumed credits
*/

-- Update stop_and_withdraw_copy_trading to handle credit record status
...
BEGIN
  ...
  IF v_is_bonus_locked THEN
    v_cost_basis := GREATEST(v_original_allocation - v_previously_withdrawn, 0);
  ELSE
    v_cost_basis := GREATEST(v_initial_balance - v_previously_withdrawn, 0);
  END IF;

  v_profit := v_current_balance - v_cost_basis;
  ...
  IF v_is_bonus_locked AND v_forfeited_amount > 0 THEN
    ...
    SET
      status = 'forfeited',
      forfeited_at = now(),
      forfeited_amount = remaining_amount,
      remaining_amount = 0,
      updated_at = now()
    WHERE relationship_id = p_relationship_id
      AND status = 'locked_in_relationship';
  ELSE
    UPDATE copy_trading_credits
    SET
      status = 'used',
