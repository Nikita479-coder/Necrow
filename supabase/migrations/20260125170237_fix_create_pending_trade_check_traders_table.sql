/*
  # Fix Create Pending Trade to Check Traders Table

  ## Summary
  Updates the create_pending_copy_trade function to also check the traders table,
  which is where featured/automated traders are stored.

  ## Changes
  - Add check for traders table in addition to admin_managed_traders and user_profiles
*/
...
BEGIN
  ...
  ELSIF EXISTS (SELECT 1 FROM traders WHERE id = p_trader_id) THEN
    v_is_admin_trader := true;
  ...
  INSERT INTO pending_copy_trades (
    ...
    COALESCE(v_actual_trader_id, p_trader_id),
    v_admin_trader_id,
    p_pair,
    p_side,
    p_entry_price,
