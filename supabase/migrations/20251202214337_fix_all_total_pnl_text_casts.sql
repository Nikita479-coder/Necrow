/*
  # Fix All total_pnl Text Casts
  
  ## Changes
  1. Fix trigger function update_cumulative_pnl_on_allocation_close
  2. Fix close_admin_trade function
  3. Ensure all total_pnl updates use numeric type
*/
...
BEGIN
  ...
  IF NEW.status = 'closed' AND OLD.status != 'closed' THEN
    -- Update the copy relationship's cumulative PNL (total_pnl is numeric)
    UPDATE copy_relationships
    SET 
      cumulative_pnl = COALESCE(cumulative_pnl, 0) + COALESCE(NEW.realized_pnl, 0),
      total_pnl = COALESCE(total_pnl, 0) + COALESCE(NEW.realized_pnl, 0),
      updated_at = NOW()
...
BEGIN
  ...
  FOR v_allocation IN
    ...
    WHERE admin_position_id = p_position_id
    AND status = 'open'
  LOOP
    ...
    BEGIN
      ...
      -- Update copy relationship cumulative PNL (keep as numeric)
      UPDATE copy_relationships
      SET 
      ...
      INSERT INTO transactions (
        ...
        v_return_amount,
        'completed',
        'Copy trade closed: ' || v_position.pair,
        NOW()
      );
    END;
  END LOOP;
