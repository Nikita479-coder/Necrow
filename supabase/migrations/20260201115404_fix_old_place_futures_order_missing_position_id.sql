/*
  # Fix Old place_futures_order Function Missing position_id

  ## Problem
  There are two versions of `place_futures_order`:
  - 9 params version (new): correctly includes position_id in fee_collections
  - 11 params version (old): MISSING position_id in fee_collections insert

  The old version is being called, causing fee_collections to have NULL position_id,
  which prevents the fee rebate trigger from checking if the trade uses locked bonus.

  This results in VIP fee rebates being incorrectly given on bonus margin trades.

  ## Solution
  1. Drop the old 11-param version
  2. Update the 9-param version to also NOT give rebates when margin is from locked bonus
*/

-- Drop the old 11-param version that's missing position_id
DROP FUNCTION IF EXISTS place_futures_order(uuid, text, text, text, integer, numeric, numeric, numeric, numeric, numeric, numeric);

-- Ensure the current 9-param version includes position_id (recreate to be sure)
  ...
  IF p_order_type = 'market' THEN
    ...
    BEGIN
      ...
      INSERT INTO transactions (
        ...
        CASE WHEN v_has_zero_fee_promo THEN 'Zero Fee Promotion Active' ELSE NULL END
      )
