/*
  # Fix Trader Performance Update - Cast to Numeric for ROUND
  
  ## Fixes
  - Cast all double precision values to numeric before ROUND
*/
...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 1..3 LOOP
      ...
      IF v_is_win THEN
        v_trade_pnl := (100 + random() * 4900)::numeric * v_trader.multiplier;
      ELSE
        v_trade_pnl := -1 * (50 + random() * 1950)::numeric * v_trader.multiplier;
      END IF;
      
      v_trade_margin := ABS(v_trade_pnl) * (2 + random()::numeric * 3);
      v_trade_quantity := v_trade_margin * v_trade_leverage;
      
      v_trade_entry := CASE 
        WHEN v_trade_pair = 'BTCUSDT' THEN (95000 + random() * 10000)::numeric
        WHEN v_trade_pair = 'ETHUSDT' THEN (3200 + random() * 400)::numeric
        WHEN v_trade_pair = 'SOLUSDT' THEN (200 + random() * 50)::numeric
        WHEN v_trade_pair = 'BNBUSDT' THEN (650 + random() * 100)::numeric
        ELSE (1 + random() * 99)::numeric
      END;
      ...
      INSERT INTO trader_trades (
        ...
        NOW() - (random()::numeric * INTERVAL '2 hours'),
        NOW(),
        NOW()
      );
    ...
    SELECT 
      ...
      COALESCE(MAX(pnl), 0),
      COALESCE(MIN(pnl), 0),
      COALESCE(AVG(leverage)::numeric, 10),
    ...
    v_roi_variance := ((random() - 0.5) * 0.30)::numeric;
    v_aum_variance := ((random() - 0.5) * 0.10)::numeric;
    v_mdd_variance := ((random() - 0.5) * 0.30)::numeric;
    v_sharpe_variance := ((random() - 0.5) * 0.40)::numeric;
    v_follower_variance := ((random() - 0.5) * 0.06)::numeric;
    ...
    v_new_pnl := COALESCE(v_trader.starting_capital, 10000000::numeric) * (v_new_roi / 100);
    v_new_aum := COALESCE(v_trader.aum, 10000000::numeric) * (1 + v_aum_variance);
    v_new_mdd := GREATEST(1::numeric, LEAST(60::numeric, COALESCE(v_trader.mdd_30d, 10::numeric) * (1 + v_mdd_variance)));
    v_new_sharpe := GREATEST(0.5::numeric, LEAST(5::numeric, COALESCE(v_trader.sharpe_ratio, 2::numeric) * (1 + v_sharpe_variance)));
    v_new_followers := GREATEST(50, ROUND(COALESCE(v_trader.followers_count, 500) * (1 + v_follower_variance))::integer);
    v_new_volatility := GREATEST(5::numeric, LEAST(30::numeric, COALESCE(v_trader.volatility_score, 15::numeric) + (random() * 6 - 3)::numeric));
    v_new_consistency := GREATEST(50::numeric, LEAST(95::numeric, COALESCE(v_trader.consistency_score, 70::numeric) + (random() * 10 - 5)::numeric));
    v_monthly_return := v_new_roi / 30 * (1 + (random() * 0.4 - 0.2)::numeric);
    ...
    SET
      ...
      avg_win_rate_7d = CASE WHEN v_total_trades > 0 THEN ROUND((v_winning_trades::numeric / v_total_trades) * 100 + (random() * 10 - 5)::numeric, 1) ELSE 50 END,
      avg_win_rate_30d = CASE WHEN v_total_trades > 0 THEN ROUND((v_winning_trades::numeric / v_total_trades) * 100 + (random() * 6 - 3)::numeric, 1) ELSE 50 END,
