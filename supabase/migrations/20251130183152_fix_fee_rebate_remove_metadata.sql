/*
  # Fix Fee Rebate Remove Metadata Column

  1. Changes
    - Update apply_fee_rebate to not use metadata column
    - Use only available transaction columns

  2. Purpose
    - Fix column not found error
    - Enable fee rebates to complete successfully
*/
...
BEGIN
  ...
  -- Skip if rebate is too small (less than $0.01)
  IF v_rebate_amount < 0.01 THEN
    RETURN 0;
  END IF;

  -- Get or create user's main wallet for USDT
