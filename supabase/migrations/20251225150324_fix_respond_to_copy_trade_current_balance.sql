/*
  # Fix respond_to_copy_trade current_balance Tracking

  ## Problem
  The respond_to_copy_trade function was ADDING allocated_amount to current_balance when 
  accepting a trade. This is incorrect because current_balance should represent the 
  AVAILABLE balance, not the total invested.

  ## Fix
  - Remove the line that adds allocated_amount to current_balance
  - current_balance should only be updated when trades CLOSE (return funds)
  - The allocated_amount is deducted from the wallet, which is the correct behavior

  ## Impact
  - Prevents double-counting of allocated amounts
  - current_balance now correctly represents available funds for new trades
*/
