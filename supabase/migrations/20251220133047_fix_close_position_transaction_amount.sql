/*
  # Fix Close Position Transaction Amount

  The transactions table requires amount > 0.
  When closing with a loss, we need to use the absolute value of the PnL.
*/
...
BEGIN
  ...
  -- Return margin from regular back to regular wallet
  ...
  IF v_pnl > 0 THEN
    -- PROFIT: Goes to regular wallet (withdrawable) minus fee
    v_profit_to_wallet := v_pnl - v_trading_fee;
    
    UPDATE futures_margin_wallets
    SET available_balance = available_balance + v_profit_to_wallet, updated_at = now()
    WHERE user_id = v_position.user_id;
    ...
    IF NOT FOUND THEN
      ...
      ON CONFLICT (user_id) DO UPDATE SET
        available_balance = futures_margin_wallets.available_balance + v_profit_to_wallet, updated_at = now();
    END IF;

    -- Track profit in oldest locked bonus
  ...
  ELSE
    ...
    UPDATE futures_margin_wallets
    SET available_balance = available_balance - v_trading_fee, updated_at = now()
    WHERE user_id = v_position.user_id;
  END IF;
  ...
  -- Use ABS for amount since constraint requires > 0, store actual PnL in details
  ...
  RETURN jsonb_build_object(
    ...
    'profit_to_wallet', round(v_profit_to_wallet, 8),
