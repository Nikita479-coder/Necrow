/*
  # Fix Close Position - No Closing Fee (Already Paid Upfront)

  ## Change
  Since we now deduct the trading fee when OPENING the position,
  we should not deduct it again when closing.
  
  The close function now only handles:
  - Returning margin to the correct source (locked bonus or futures wallet)
  - Calculating and applying PnL
*/
...
BEGIN
  ...
  IF v_margin_from_locked > 0 THEN
    ...
    ELSE
      -- No active locked bonus anymore, credit to futures wallet instead
  ...
  -- STEP 3: Handle PnL (no fee deduction - already paid on open)
  IF v_pnl >= 0 THEN
    -- PROFIT: Credit full profit to futures wallet
    UPDATE futures_margin_wallets
    SET available_balance = available_balance + v_pnl,
        updated_at = now()
    WHERE user_id = v_position.user_id;

    IF NOT FOUND THEN
      INSERT INTO futures_margin_wallets (user_id, available_balance, locked_balance)
      VALUES (v_position.user_id, v_pnl, 0)
      ON CONFLICT (user_id) DO UPDATE SET
        available_balance = futures_margin_wallets.available_balance + v_pnl,
        updated_at = now();
    END IF;

    -- Track profit in locked bonus
    IF v_oldest_bonus_id IS NOT NULL THEN
      UPDATE locked_bonuses 
