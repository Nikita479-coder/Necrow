/*
  # Fix Admin Check with Profile Fallback

  ## Overview
  Updates `create_pending_copy_trade` to check both JWT metadata AND user_profiles table
  for admin status. This ensures admins can create trades even if their JWT hasn't refreshed yet.

  ## Changes
  - Check JWT metadata first (fastest)
  - Fall back to checking user_profiles.is_admin if JWT doesn't have the flag
  - This allows existing sessions to work without requiring logout/login

  ## Security
  - Maintains all existing security checks
  - Uses SECURITY DEFINER so it can read user_profiles
  - Only affects admin authorization, not other security measures
*/
