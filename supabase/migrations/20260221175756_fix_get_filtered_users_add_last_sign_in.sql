/*
  # Fix get_filtered_users to include last_sign_in_at

  1. Changes
    - Drop old json-parameter version of get_filtered_users
    - Recreate with json parameter type to match frontend calls
    - Add last_sign_in_at from auth.users table
    - Include full_name, email, and other missing fields

  2. Notes
    - The frontend passes json (not jsonb), so we replace the json version
    - Also drops the jsonb version to avoid ambiguity
*/
