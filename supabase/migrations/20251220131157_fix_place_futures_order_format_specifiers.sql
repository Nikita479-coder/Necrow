/*
  # Fix format specifiers in place_futures_order
  
  PostgreSQL format() only supports %s, %I, %L - not %.2f like C.
  Changed to use %s with round() for numeric formatting.
*/
...
BEGIN
  ...
  IF v_available_balance IS NULL OR v_available_balance < v_total_cost THEN
    RETURN jsonb_build_object(
      'success', false, 
      'error', 'Insufficient balance. Required: $' || round(v_total_cost, 2) || ', Available: $' || round(COALESCE(v_available_balance, 0), 2)
    );
  END IF;
