/*
  # Fix Locked Bonus Profits - Go to Bonus, Not Wallet
  
  1. Problem
    - When trades using locked bonus funds profit, the profit was going to the regular futures wallet
    - This allowed users to extract value from locked bonuses without meeting unlock requirements
    - Only losses were being deducted from the locked bonus
  
  2. Solution
    - Profits from locked bonus trades should increase the locked bonus current_amount
    - Losses from locked bonus trades should decrease the locked bonus current_amount
    - The margin always returns to where it came from
    - Only margin from regular wallet goes back to regular wallet
  
  3. Changes
    - Update close_position function to route profits correctly based on margin source
*/
...
BEGIN
  ...
  -- Calculate how to split PnL between locked bonus and regular wallet
  -- PnL is distributed proportionally based on margin source
  ...
  -- Handle locked bonus portion: margin + proportional PnL goes back to locked bonus
  IF v_margin_from_locked > 0 THEN
    ...
    IF v_oldest_bonus_id IS NOT NULL THEN
      -- Return margin AND proportional PnL to locked bonus
      -- If profit: locked bonus grows; If loss: locked bonus shrinks
      ...
      -- Volume tracking only if duration requirement met
  ...
  -- Track real volume for VIP/referral
  ...
  -- Handle regular wallet portion: margin + proportional PnL
  -- Only unlock from locked_balance, return margin to available
  ...
  -- Add margin from regular wallet back to available balance
  ...
  -- Add PnL from regular portion to futures wallet
  ...
  ELSIF v_pnl_to_regular < 0 THEN
    -- Deduct loss from futures wallet (but don't go negative)
    UPDATE futures_margin_wallets
