/*
  # Create place_futures_order function
  
  Creates the main entry point function for placing futures orders that the frontend expects.
  Handles both market and limit orders with all required parameters.
  
  Parameters:
    - p_user_id: User placing the order
    - p_pair: Trading pair (e.g., BTCUSDT)
    - p_side: 'long' or 'short'
    - p_order_type: 'market' or 'limit'
    - p_quantity: Position size
    - p_leverage: Leverage multiplier
    - p_margin_mode: 'cross' or 'isolated'
    - p_price: Limit price (required for limit orders)
    - p_stop_loss: Optional stop loss price
    - p_take_profit: Optional take profit price
    - p_trigger_price: For stop orders
    - p_reduce_only: Whether this reduces an existing position
*/
...
BEGIN
  ...
  -- Get user leverage limit
  SELECT max_leverage INTO v_user_leverage_limit
  ...
  FROM trading_pairs
  WHERE symbol = p_pair AND is_active = true;
  ...
  v_margin_amount := (p_quantity * v_entry_price) / p_leverage;
  v_fee_amount := (p_quantity * v_entry_price) * v_fee_rate;
  ...
  IF p_order_type = 'market' THEN
    ...
    INSERT INTO fee_collections (
      user_id, fee_type, pair, fee_amount, fee_currency
    ) VALUES (
      p_user_id, 'futures_open', p_pair, v_fee_amount, 'USDT'
    );

    -- Create position
    INSERT INTO futures_positions (
      user_id, pair, side, entry_price, quantity, leverage,
      margin_mode, margin_used, stop_loss, take_profit, status
    ) VALUES (
  ...
  ELSE
    ...
    INSERT INTO futures_orders (
      ...
      margin_mode, stop_loss, take_profit, status, margin_reserved
    ) VALUES (
    ...
    UPDATE futures_margin_wallets
    SET balance = balance - v_margin_amount,
        updated_at = now()
    WHERE user_id = p_user_id;
