/*
  # Consolidate transfer_between_wallets Functions

  1. Problem
    - Three overloaded versions of transfer_between_wallets exist with different parameter names
    - The frontend calls version 1 (user_id_param signature) which does NOT auto-allocate to copy relationships
    - Version 3 (p_user_id with p_amount before p_currency) HAS auto-allocation but is unreachable
    - This caused funds to go to the copy wallet but not be allocated to active copy relationships

  2. Changes
    - Drop the two unused overloads (oid 208320 and 321133)
    - Update the version the frontend calls (user_id_param signature) to include auto-allocation
    - When transferring TO copy wallet and exactly 1 active real relationship exists, auto-allocate

  3. Auto-Allocation Logic
    - After crediting the copy wallet, check if destination is 'copy'
    - Count active real (non-mock) copy relationships
    - If exactly 1, add the transferred amount to that relationship's initial_balance and current_balance
    - Create a copy_topup transaction for audit trail
*/

-- Step 1: Drop the two unused overloads
DROP FUNCTION IF EXISTS transfer_between_wallets(uuid, text, text, text, numeric);
DROP FUNCTION IF EXISTS transfer_between_wallets(uuid, text, text, numeric, text);

-- Step 2: Recreate the version the frontend calls with auto-allocation
...
BEGIN
  ...
  ELSIF from_wallet_type_param = 'copy' THEN
    ...
    v_actual_available := COALESCE(v_from_balance, 0) - v_from_locked - v_allocated_to_traders;

    IF v_actual_available < amount_param THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', format('Insufficient available balance. Available: $%.2f (Total: $%.2f, Allocated: $%.2f)',
          v_actual_available, v_from_balance, v_allocated_to_traders)
      );
    END IF;
  ...
  -- === AUTO-ALLOCATE TO ACTIVE COPY RELATIONSHIP ===
