/*
  # Fix Audience Counts - Copy Relationships Column

  1. Fixes
    - Use correct column name 'follower_id' instead of 'copier_id'
*/
...
BEGIN
  ...
  -- Copy Traders (users following traders)
  SELECT COUNT(DISTINCT follower_id) INTO v_count
  ...
  FROM user_profiles up
  WHERE NOT EXISTS (
    SELECT 1 FROM copy_relationships cr WHERE cr.follower_id = up.id AND cr.status = 'active'
  );
