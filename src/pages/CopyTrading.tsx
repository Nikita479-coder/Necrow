...
function CopyTrading() {
  ...
  const loadMockCopies = async () => {
    ...
    try {
      ...
      if (data) {
        const relationshipIds = data.map(c => c.id);
        let openMarginByRelationship: Record<string, number> = {};
        if (relationshipIds.length > 0) {
          const { data: allocData } = await supabase
            .from('copy_trade_allocations')
            .select('copy_relationship_id, allocated_amount')
            .in('copy_relationship_id', relationshipIds)
            .eq('status', 'open');
          if (allocData) {
            for (const a of allocData) {
              openMarginByRelationship[a.copy_relationship_id] = (openMarginByRelationship[a.copy_relationship_id] || 0) + parseFloat(String(a.allocated_amount || 0));
            }
          }
        }
        ...
        const formattedCopiesPromises = data.map(async (copy) => {
          const initialBalance = parseFloat(copy.initial_balance || '0');
          const currentBalance = parseFloat(copy.current_balance || '0');
          const openMargin = openMarginByRelationship[copy.id] || 0;
          const effectiveBalance = currentBalance + openMargin;

          let userRoi30d = 0;
          if (initialBalance > 0) {
            userRoi30d = ((effectiveBalance - initialBalance) / initialBalance) * 100;
          }
