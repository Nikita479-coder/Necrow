...
function ActiveCopyTrading() {
  ...
  const getOpenMarginTotal = () => {
    if (!selectedCopy?.is_mock) return 0;
  ...
  const getEffectiveBalance = () => {
    ...
    return current + getOpenMarginTotal();
  };
  ...
  const calculateROI = () => {
    ...
    return (((getEffectiveBalance() - initial) / initial) * 100).toFixed(2);
  };

  const calculateProfit = () => {
    if (!selectedCopy) return 0;
    return getEffectiveBalance() - parseFloat(selectedCopy.initial_balance);
  };
        ...
        <div className="bg-gradient-to-r from-[#1e2329] to-[#252930] rounded-2xl border border-[#2b3139] overflow-hidden mb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-px bg-[#2b3139]">
            ...
            <div className="bg-[#1e2329] p-4 sm:p-6">
              ...
              <div className="text-white text-lg sm:text-2xl font-bold">
                {getEffectiveBalance().toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                <span className="text-[#848e9c] text-sm ml-1">USDT</span>
              </div>
              {selectedCopy.is_mock && getOpenMarginTotal() > 0 && (
                <div className="text-[#848e9c] text-xs mt-1">
                  {getOpenMarginTotal().toFixed(2)} in open positions
                </div>
              )}
            </div>
