...
import { AlertTriangle, Shield, Clock, CheckCircle2, Search, Info, Send, Users, Zap, RefreshCw, Lock, MessageCircle, TrendingUp, Smartphone } from 'lucide-react';
...
function Withdraw() {
  ...
  const [copyTradingCredits, setCopyTradingCredits] = useState<{ total_credit: number; locked_credit: number } | null>(null);
  ...
  const loadCopyTradingCredits = async () => {
    if (!user) return;

    try {
      const { data, error } = await supabase.rpc('get_user_copy_trading_credits');
      ...
      console.error('Error loading copy trading credits:', error);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {copyTradingCredits && (copyTradingCredits.total_credit > 0 || copyTradingCredits.locked_credit > 0) && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-red-400 mb-1">Copy Trading Bonus Warning</p>
                    <p className="text-xs text-red-400 mb-2">
                      You have ${(copyTradingCredits.total_credit + copyTradingCredits.locked_credit).toFixed(2)} in copy trading bonus funds active.
                      Withdrawing from your main wallet will not affect your bonus, but stopping copy trading will forfeit the bonus.
                    </p>
                    <p className="text-xs text-gray-400">
                      To protect your bonus: Keep your copy trading active or first withdraw from copy wallet only.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className={`bg-[#181a20] border border-gray-800 rounded-2xl overflow-hidden ${bonusStatus?.withdrawal_blocked ? 'opacity-40 pointer-events-none select-none' : ''}`}>
              ...
              <div className="p-6 space-y-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-400 text-xs mt-1">
                          Enter the 6-digit code from your authenticator app before submitting your withdrawal request.
                        </p>
                      </div>
                    </div>
                ...
                <button
                  ...
                  disabled={!isExternalFormValid || isWithdrawing || (needsMfaForWithdraw && withdrawMfaCode.length !== 6)}
                ...
                >
                  {isWithdrawing ? (
                    ...
                    <>
                      <Send className="w-5 h-5" />
                      {needsMfaForWithdraw ? `Verify & Withdraw ${selectedCrypto}` : `Withdraw ${selectedCrypto}`}
                    </>
                  )}
                </button>
