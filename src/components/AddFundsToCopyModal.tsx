  ...
  const fetchBalances = async () => {
    ...
    try {
      ...
      const { data: allActiveRelationships } = await supabase
        ...
        .select('id, initial_balance, cumulative_pnl')
      ...
      const totalAllocated = allActiveRelationships?.reduce(
        (sum, rel) => {
          const initial = parseFloat(rel.initial_balance || '0');
          const pnl = parseFloat(rel.cumulative_pnl || '0');
          return sum + Math.max(0, initial + pnl);
        },
        0
      ) || 0;
