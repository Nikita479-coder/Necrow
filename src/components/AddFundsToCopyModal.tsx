...
export default function AddFundsToCopyModal({
  ...
  const fetchBalances = async () => {
    ...
    try {
      ...
      const totalAllocated = allActiveRelationships?.reduce(
        (sum, rel) => sum + Math.max(0, parseFloat(rel.current_balance || '0')),
        0
      ) || 0;
  ...
  const handleAddFunds = async () => {
    ...
    if (!isMock && addAmount > Math.round(availableBalance * 100) / 100) {
          ...
          <div className="space-y-4">
            <div>
              ...
              {amount && parseFloat(amount) > 0 && parseFloat(amount) < 10 && (
                <p className="text-[#f6465d] text-xs mt-1">Minimum amount is 10 USDT</p>
              )}
              {!isMock && amount && parseFloat(amount) > Math.round(availableBalance * 100) / 100 && (
                <p className="text-[#f6465d] text-xs mt-1">Amount exceeds available balance ({availableBalance.toFixed(2)} USDT)</p>
              )}
            </div>
            ...
            <button
              ...
              disabled={loading || !amount || parseFloat(amount) < 10 || (!isMock && parseFloat(amount) > Math.round(availableBalance * 100) / 100)}
