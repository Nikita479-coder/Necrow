...
export default function WalletTransferModal({
  ...
  const [copyTradingCredits, setCopyTradingCredits] = useState<CopyTradingCredits | null>(null);
  ...
  const loadCopyTradingCredits = useCallback(async () => {
    if (!user) return;

    try {
      const { data, error: rpcError } = await supabase.rpc('get_user_copy_trading_credits');

      if (rpcError) throw rpcError;

      if (data && data.success) {
        setCopyTradingCredits({
          available_credit: data.available_credit || 0,
          total_credit: data.total_credit || 0,
          locked_credit: data.locked_credit || 0
        });
      } else {
        setCopyTradingCredits(null);
      }
    } catch (err) {
      console.error('Error loading copy trading credits:', err);
      setCopyTradingCredits(null);
    }
  }, [user]);
  ...
  useEffect(() => {
    if (isOpen && user) {
      loadBalances();
      loadCopyTradingCredits();
      setError('');
      setSuccess('');
      setAmount('');
    }
  }, [isOpen, user, loadBalances, loadCopyTradingCredits]);
  ...
  const protectedCopyCredit = fromWallet === 'copy' ? (copyTradingCredits?.available_credit || 0) : 0;
  const transferableAvailable = fromWallet === 'copy'
    ? Math.max(currentAvailable - protectedCopyCredit, 0)
    : currentAvailable;
  const hasProtectedCopyCredit = fromWallet === 'copy' && protectedCopyCredit > 0;
  ...
  const handleTransfer = async () => {
    ...
    if (transferAmount > transferableAvailable) {
      setError(fromWallet === 'copy'
        ? 'Only real funds can be transferred from the copy wallet. Bonus credit funds must remain in the copy wallet.'
  ...
  const handleMaxClick = () => {
    if (transferableAvailable > 0) {
      setAmount(transferableAvailable.toFixed(8).replace(/\.?0+$/, ''));
    }
  };
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#0b0e11] border border-gray-700 rounded-xl p-4">
              ...
              {hasProtectedCopyCredit && (
                <div className="text-xs text-red-400 mt-1">
                  {protectedCopyCredit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDT bonus credit protected
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={transferableAvailable <= 0}
            ...
            <p className="text-xs text-gray-500 mt-2">
              Available: {transferableAvailable.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 8 })} USDT
            </p>
          </div>
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-red-400 text-sm font-medium">
                  Copy trading bonus funds are active in this wallet
                </p>
                <p className="text-gray-400 text-xs mt-1">
                  ${protectedCopyCredit.toFixed(2)} USDT is bonus credit and cannot be transferred. You can transfer only your real available funds.
                </p>
              </div>
            </div>
          )}
          ...
          <button
            ...
            disabled={loading || !amount || parseFloat(amount) <= 0 || parseFloat(amount) > transferableAvailable || success !== ''}
