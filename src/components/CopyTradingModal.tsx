...
export default function CopyTradingModal({
    ...
    const fetchBalance = async () => {
      ...
      try {
        if (isMock) {
          ...
          const { data: creditData } = await supabase.rpc('get_user_copy_trading_credits');
          if (creditData) {
            setCreditBalance(creditData.available_credit || 0);
            const credits = creditData.credits || [];
            if (credits.length > 0) {
              setCreditLockDays(credits[0].lock_days || 30);
            }
          }
  ...
  const handleStartCopy = async () => {
    ...
    const calculatedAmount = availableBalance !== null
      ...
      : 0;
    const effectiveMinimum = creditBalance > 0 ? Math.max(Math.min(creditBalance, 100), 100) : 100;

    if (!isMock && calculatedAmount < effectiveMinimum) {
      setError(`Minimum copy amount is ${effectiveMinimum} USDT. Please increase your allocation percentage or add funds to your wallet.`);
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-[#1e2329] rounded-lg w-full max-w-md max-h-[90vh] flex flex-col relative">
        ...
        {!isMock && creditBalance > 0 && (
          <div className="bg-[#0ecb81]/10 border border-[#0ecb81]/30 rounded-lg p-3 mb-4">
            <div className="flex items-start gap-2">
              <Gift className="w-4 h-4 text-[#0ecb81] flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="text-[#0ecb81] text-sm font-medium">
                  You have ${creditBalance.toFixed(2)} in copy trading credits
                </p>
                <p className="text-[#0ecb81]/70 text-xs mt-1">
                  Credits are applied automatically and locked for {creditLockDays} days. Withdrawing before {creditLockDays} days forfeits the credit portion and its profits.
                </p>
              </div>
            </div>
          </div>
              ...
              {creditBalance > 0 && (
                <div className="flex items-center justify-between pt-2 border-t border-[#2b3139]">
                  <div className="flex items-center gap-1">
                    <Gift className="w-3 h-3 text-[#0ecb81]" />
                    <span className="text-[#0ecb81] text-xs">Copy Trading Credit</span>
                  </div>
                  <span className="text-[#0ecb81] text-xs font-medium">
        ...
        {!isMock && availableBalance !== null && availableBalance > 0 &&
         calculatedAmount < effectiveMinimum && (
          <div className="bg-[#f6465d]/10 border border-[#f6465d]/30 rounded-lg p-3 mb-4 flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-[#f6465d] flex-shrink-0 mt-0.5" />
            <span className="text-[#f6465d] text-sm">Minimum copy amount is {effectiveMinimum} USDT. Increase your allocation percentage or transfer more funds.</span>
          </div>
        )}
