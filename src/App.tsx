...
function App() {
  ...
  useEffect(() => {
    ...
    if (pathname === '/link-bot') {
      setCurrentPage('linkbot');
      return;
    }
    if (pathname === '/login-with-shark') {
      setCurrentPage('loginwithshark');
      return;
    }
  ...
  const getPageTitle = (page: PageType): string => {
    const titles: Record<PageType, string> = {
      ...
      linkbot: 'Link Game Bot',
      loginwithshark: 'Login with Shark'
    };
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'linkbot':
        return wrapWithTracker(<LinkBot />);
      case 'loginwithshark':
        return wrapWithTracker(<LoginWithShark />);
      default:
        return wrapWithTracker(<HomePage />);
    }
  };

  const isStandalonePage = currentPage === 'lp' || currentPage === 'linkbot' || currentPage === 'loginwithshark';

  return (
    <AuthProvider>
      <NavigationContext.Provider value={{ currentPage, navigateTo, navigationState }}>
        {!isStandalonePage && <PopupBanner />}
        {renderPage()}
        <CookieConsentBanner />
